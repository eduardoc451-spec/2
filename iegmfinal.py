...
# ==============================================================================
# CONFIGURAÇÃO DIRETA DO POSTGRESQL (Preencha com seus dados)
# ==============================================================================
PG_HOST = "localhost"        # IP ou Host do banco
PG_PORT = 5432               # Porta padrão
PG_DBNAME = "seu_banco"      # Nome do banco de dados
PG_USER = "postgres"         # Usuário
PG_PASSWORD = "sua_senha"    # Senha
# ==============================================================================
...
class SistemaHAL:
    ...
    def inicializar_banco_postgres(_self):
        """Conecta diretamente no PostgreSQL usando as variáveis declaradas no início"""
        try:
    ...
    def _obter_nome_tabela(self, dimensao: str) -> str:
        ...
        return self.mapeamento_tabelas.get(dimensao, f"respostas_{dimensao.lower().replace('-', '')}")

    def puxar_nota_dimensao(self, dimensao: str, ano: int) -> float:
        """
        Calcula a nota aplicando exatamente a regra TCESP do seu modelo:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                try:
                    ...
                    conn.rollback()
                    sql = "SELECT pontos FROM respostas WHERE LOWER(dimensao) = %s AND ano = %s;"
            ...
            logging.warning(f"[{dimensao}] Erro ao ler nota do ano {ano}: {e}")
            return 0.0
    ...
    def consultar_anos(self, dimensao: str, quesito_id: str):
        ...
        try:
            with conn.cursor() as cursor:
                try:
                    cursor.execute(
                        f"SELECT ano, valor FROM {tabela} WHERE id = %s ORDER BY ano ASC;",
                        (quesito_id,)
                    )
                    ...
                    conn.rollback()
                    cursor.execute(
                        "SELECT ano, valor FROM respostas WHERE LOWER(dimensao) = %s AND id = %s ORDER BY ano ASC;",
    ...
    def analisar_pontos_fracos(self, ano: int, dimensao: str):
        ...
        try:
            with conn.cursor() as cursor:
                try:
                    cursor.execute(
                        f"SELECT id, valor, pontos FROM {tabela} WHERE ano = %s;",
                        (int(ano),)
                    )
                    rows = cursor.fetchall()
                except Exception:
                    conn.rollback()
                    cursor.execute(
                        "SELECT id, valor, pontos FROM respostas WHERE LOWER(dimensao) = %s AND ano = %s;",
                        (dimensao.lower(), int(ano))
                    )
                    rows = cursor.fetchall()
                
                for qid, valor, pontos_reais in rows:
                    pontos_reais = float(pontos_reais) if pontos_reais is not None else 0.0
                    if qid in questoes:
                        if pontos_reais < 0:
    ...
    def calcular_evolucao_pontos(self, dimensao: str):
        anos_validos = [2023, 2024, 2025, 2026, 2027]
        dados_anos = []
        
        max_dim = sum(self.pontuacoes_maximas_por_dimensao.get(dimensao, {}).values()) or 100.0

        for ano in anos_validos:
            nota_liquida = self.puxar_nota_dimensao(dimensao, ano)
            p_perc = (nota_liquida / max_dim) * 100.0
...
def mostrar_chat_hal():
    ...
    dimensoes_disponiveis = ["iGov-Ti", "iCidade", "i-Amb", "i-Plan", "i-Fiscal", "i-Educ", "i-Saude"]
    ...
    st.markdown(
        ...
        <div class="chat-bubble-ia">
            ...
            <div class="content-ia">
                Roteamento concluído com sucesso. Conectado ao <b>PostgreSQL ({PG_HOST})</b> lendo a tabela <b>{sistema._obter_nome_tabela(dimensao_selecionada)}</b> com a regra TCESP ativa para <b>{dimensao_selecionada}</b>.
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if historico_performance:
        cols = st.columns(len(historico_performance))
        for idx, item in enumerate(historico_performance):
            with cols[idx]:
                st.metric(
                    label=f"Ano {item['ano']}",
                    value=f"{item['liquido']:.1f} pts",
                    delta=f"Faixa {item['faixa']}"
                )
