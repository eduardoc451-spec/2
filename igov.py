...
# =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_7_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.0 - Serviço de Informação ao Cidadão (e-SIC)", expanded=True):
            ...
            # Sanitização e mapeamento de dados legados
            ...
            evidencia_70_salva = d70.get("link", "") or ""
            chave_radio_70 = f"r_70_select_{ano_sel}"
            chave_txt_70 = f"l_70_txt_area_{ano_sel}"

            def cb_processa_e_salva_70():
                lnk_val = (st.session_state.get(chave_txt_70) or "").strip()
                ...
                links_atuais = re.findall(regex_pure_url, lnk_val)
                links_antigos = re.findall(regex_pure_url, evidencia_70_salva)
    ...
    with st.container(key=f"container_bloco_compdec_7_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.1 - Simplificação de Exigências Cadastrais", expanded=True):
            ...
            evidencia_71_salva = d71.get("link", "") or ""
            chave_radio_71 = f"r_71_select_{ano_sel}"
            chave_txt_71 = f"l_71_txt_area_{ano_sel}"

            def cb_processa_e_salva_71():
                lnk_val = (st.session_state.get(chave_txt_71) or "").strip()
                ...
                links_atuais = re.findall(regex_pure_url, lnk_val)
                links_antigos = re.findall(regex_pure_url, evidencia_71_salva)
    ...
    with st.container(key=f"container_bloco_compdec_7_2_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.2 - Mecanismo de Acompanhamento de Pedidos", expanded=True):
            ...
            evidencia_72_salva = d72.get("link", "") or ""
            chave_radio_72 = f"r_72_select_{ano_sel}"
            chave_txt_72 = f"l_72_txt_area_{ano_sel}"

            def cb_processa_e_salva_72():
                lnk_val = (st.session_state.get(chave_txt_72) or "").strip()
                ...
                links_atuais = re.findall(regex_pure_url, lnk_val)
                links_antigos = re.findall(regex_pure_url, evidencia_72_salva)
