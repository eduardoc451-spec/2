...
st.set_page_config(
    layout="wide", page_title="Plano de Ação IEG-M", page_icon="🎯"
)
...
def carregar_dados_locais():
      ...
      with open(ARQUIVO_BANCO, "r", encoding="utf-8") as f:
        ...
        for item in dados:
          if item.get("data_inicio"):
            item["data_inicio"] = datetime.strptime(
                item["data_inicio"], "%Y-%m-%d"
            ).date()
          if item.get("data_conclusao"):
            item["data_conclusao"] = datetime.strptime(
                item["data_conclusao"], "%Y-%m-%d"
            ).date()
        return dados
  ...
  return [{
      ...
      "descricao_acao": (
          "Medição de todas as salas de aula para otimização de espaço físico."
      ),
      "fragilidades": (
          "Falta de engenheiro permanente na secretaria para homologar laudos."
      ),
  ...
  try:
    ...
    for item in st.session_state.plano_acao_db:
      ...
      if isinstance(item_copia.get("data_inicio"), date):
        item_copia["data_inicio"] = item_copia["data_inicio"].strftime(
            "%Y-%m-%d"
        )
      if isinstance(item_copia.get("data_conclusao"), date):
        item_copia["data_conclusao"] = item_copia["data_conclusao"].strftime(
            "%Y-%m-%d"
        )
...
def gerar_pdf_relatorio(df_dados, ano_selecionado):
  ...
  if os.path.exists("iegm.png"):
    try:
      ...
      story.append(
          Paragraph("[Erro ao renderizar arquivo iegm.png]", style_capa_sub)
      )
  else:
    story.append(
        Paragraph(
  ...
  for d_item in dimensoes_presentes:
    texto_sumario = (
        f"• Dimensão Temática: <b>{d_item.upper()}</b>"
        " ............................................................................................................"
        " Ver Seção Detalhada"
    )
    ...
    for _, row in df_dim.iterrows():
      dados_tabela = [
          ...
          [
              ...
              Paragraph(str(row["meta_estrategica"]), style_texto_normal),
          ...
          [
              ...
              Paragraph(str(row["resultados_esperados"]), style_texto_normal),
          ],
      ]
...
def mostrar_formulario_plano_acao():
  ...
  filtro_dim = st.sidebar.selectbox(
      "Filtrar por Dimensão IEG-M", ["Todas"] + dimensoes_iegm
  )
  ...
  titulo_pagina_dinamico = (
      f"🎯 Painel Estratégico do Plano de Ação — {filtro_ano}"
      if filtro_ano != "Todos"
      else "🎯 Painel Estratégico do Plano de Ação — Plurianual"
  )
  st.title(titulo_pagina_dinamico)
  st.caption(
      f"💾 Banco de Dados Físico Ativo Localmente em: `{ARQUIVO_BANCO}`"
  )

  df_filtrado = df_completo.copy()

  # Extração segura e isolada do ano
  df_filtrado["ano_conclusao_aux"] = df_filtrado["data_conclusao"].apply(
  ...
  if filtro_ano != "Todos":
    df_filtrado = df_filtrado[
        df_filtrado["ano_conclusao_aux"] == int(filtro_ano)
    ]

  # CARDS VISUAIS DE PERFORMANCE (KPIs)
  totais = (
      df_filtrado["status"].value_counts()
  ...
  with m2:
    st.markdown(
        f"<div style='border-left: 5px solid #28a745; padding-left:"
        " 10px;'><strong>Atendidas</strong><h2 style='color:#28a745;"
        f" margin:0;'>{qtd_verde}</h2></div>",
        unsafe_allow_html=True,
    )
  with m3:
    st.markdown(
        f"<div style='border-left: 5px solid #ffc107; padding-left:"
        " 10px;'><strong>Em Análise</strong><h2 style='color:#ffc107;"
        f" margin:0;'>{qtd_amarelo}</h2></div>",
        unsafe_allow_html=True,
    )
  with m4:
    st.markdown(
        f"<div style='border-left: 5px solid #dc3545; padding-left:"
        " 10px;'><strong>Pendentes (Riscos)</strong><h2 style='color:#dc3545;"
        f" margin:0;'>{qtd_vermelho}</h2></div>",
        unsafe_allow_html=True,
    )
  ...
  if not df_filtrado.empty:
    ...
    rotulo_botao_pdf = (
        f"📄 Baixar Plano de Ação - Ano {filtro_ano} (PDF Oficial)"
        if filtro_ano != "Todos"
        else "📄 Baixar Plano de Ação - Plurianual (PDF Oficial)"
    )
    st.download_button(
        label=rotulo_botao_pdf,
        data=pdf_data,
        file_name=(
            f"Plano_de_Acao_IEGM_Ano_{filtro_ano}_{date.today().strftime('%Y%m%d')}.pdf"
        ),
    ...
    st.button(
        "📄 Gerar Relatório PDF Consolidado (Filtros sem resultados)",
        disabled=True,
        use_container_width=True,
    )
  ...
  with tab_dashboard:
    ...
    if df_filtrado.empty:
      ...
      for index, row in df_filtrado.iterrows():
        cor_status = (
            "#28a745"
            if "Verde" in row["status"]
            else "#ffc107"
            if "Amarelo" in row["status"]
            else "#dc3545"
        )
        ...
        with st.container(border=True):
          ...
          with c_cab2:
            st.markdown(
                f"<div style='text-align: right; font-weight: bold; color:"
                f" {cor_status};'>{row['status']}</div>",
                unsafe_allow_html=True,
            )

          st.markdown(
              "<hr style='margin: 10px 0; border: 0; border-top: 1px solid"
          ...
          with bloco_execucao:
            ...
            st.markdown(
                f"**⚠️ Fragilidades:** <span style='color:#dc3545;"
                f" font-weight:bold;'>{row['fragilidades']}</span>",
                unsafe_allow_html=True,
            )
          ...
          with bloco_governanca:
            ...
            st.markdown(
                "**📅 Prazo Final:**"
                f" {row['data_conclusao'].strftime('%d/%m/%Y') if row['data_conclusao'] else 'Não definido'}"
            )
            ...
            if links_texto:
              lista_links = [
                  lnk.strip() for lnk in links_texto.split(",") if lnk.strip()
              ]
              for idx_link, link_url in enumerate(lista_links, start=1):
                st.link_button(
                    f"🔗 Evidência {idx_link}",
                    link_url,
                    use_container_width=True,
                )

          with st.expander("🛠️ Editar / Remover Registro"):
            with st.form(f"form_editar_{index}"):
              ed_c1, ed_c2, ed_c3 = st.columns(3)
              with ed_c1:
                nova_dim = st.selectbox(
                    "Dimensão IEG-M",
                    dimensoes_iegm,
                    index=dimensoes_iegm.index(row["dimensao"]),
                )
                nova_meta_est = st.text_input(
                    "Meta Estratégica", value=row["meta_estrategica"]
                )
                novo_ind = st.text_input(
                    "Indicador de Desempenho", value=row["indicador_desempenho"]
                )
                nova_acao = st.text_input(
                    "Título Prático da Ação", value=row["acao"]
                )
                novo_status = st.selectbox(
                    "Status Atual",
                    lista_status,
                    index=lista_status.index(row["status"]),
                )
              with ed_c2:
                nova_desc = st.text_area(
                    "Descrição da Operação", value=row["descricao_acao"]
                )
                novas_frag = st.text_area(
                    "Fragilidades Identificadas", value=row["fragilidades"]
                )
                nova_meta_alvo = st.text_input(
                    "Meta Alvo", value=row["meta"]
                )
                novo_resultados = st.text_input(
                    "Resultados Esperados", value=row["resultados_esperados"]
                )
              with ed_c3:
                nova_dt_ini = st.date_input(
                    "Início da Execução",
                    value=row["data_inicio"],
                    format="DD/MM/YYYY",
                    key=f"ini_{index}",
                )
                nova_dt_fim = st.date_input(
                    "Prazo Limite",
                    value=row["data_conclusao"],
                    format="DD/MM/YYYY",
                    key=f"fim_{index}",
                )
                novo_report = st.selectbox(
                    "Período de Report",
                    periodos_report,
                    index=periodos_report.index(row["periodo_report"]),
                )
                novo_resp = st.text_input(
                    "Responsável Principal", value=row["responsavel"]
                )
                nova_forma = st.text_input(
                    "Forma de Execução", value=row["forma_execucao"]
                )
                novas_evid = st.text_input(
                    "Evidências (Texto)", value=row["evidencias"]
                )
                novo_links_evid = st.text_input(
                    "🔗 Links das Evidências (Separados por vírgula)",
                    value=row["links_evidencias"],
                )
                nova_integ = st.text_input(
                    "Integração Planejamento",
                    value=row["integracao_planejamento_municipal"],
                )
                novo_ods = st.text_input(
                    "Alinhamento ODS", value=row["alinhamento_ods"]
                )
              ...
              if btn_atualizar:
                st.session_state.plano_acao_db[index] = {
                    "dimensao": nova_dim,
                    "meta_estrategica": nova_meta_est,
                    "indicador_desempenho": novo_ind,
                    "acao": nova_acao,
                    "descricao_acao": nova_desc,
                    "fragilidades": novas_frag,
                    "meta": nova_meta_alvo,
                    "resultados_esperados": novo_resultados,
                    "integracao_planejamento_municipal": nova_integ,
                    "alinhamento_ods": novo_ods,
                    "data_inicio": nova_dt_ini,
                    "data_conclusao": nova_dt_fim,
                    "periodo_report": novo_report,
                    "responsavel": novo_resp,
                    "forma_execucao": nova_forma,
                    "evidencias": novas_evid,
                    "links_evidencias": novo_links_evid,
                    "status": novo_status,
                }
                salvar_dados_locais()
  ...
  # ABA 2: CONSOLIDAÇÃO DE METAS E CONTAGEM
  with tab_metas:
    st.markdown("### 🎯 Metas Estratégicas e Volume de Ações")
    if df_filtrado.empty:
      st.info(
          "Nenhum dado disponível para consolidar metas com os filtros atuais."
      )
    else:
      df_agrupado_metas = (
          df_filtrado.groupby("meta_estrategica")
          .size()
          .reset_index(name="Total de Ações")
      )
      df_agrupado_metas = df_agrupado_metas.sort_values(
          by="Total de Ações", ascending=False
      )

      st.dataframe(
          df_agrupado_metas,
          column_config={
              "meta_estrategica": st.column_config.TextColumn(
                  "🎯 Meta Estratégica"
              ),
              "Total de Ações": st.column_config.NumberColumn(
                  "📋 Total de Ações Associadas", format="%d"
              ),
          },
          use_container_width=True,
          hide_index=True,
  ...
  # ABA 3: GRÁFICO EM PIZZA DOS STATUS
  with tab_graficos:
    st.markdown("### 📊 Distribuição e Percentual por Status")
    if df_filtrado.empty:
      st.info(
          "Insira dados ou limpe os filtros para visualizar os gráficos de"
          " performance."
      )
    else:
      ...
      fig.update_traces(
          ...
          hovertemplate=(
              "%{label}<br>Total: %{value} ações<br>Percentual: %{percent}"
          ),
      )
      fig.update_layout(
      ...
      with c_graf2:
        ...
        for _, r_graf in df_status_chart.iterrows():
          ...
          st.write(
              f"- **{r_graf['Status']}**: {r_graf['Quantidade']} ações"
              f" ({pct:.1f}%)"
          )

  # ABA 4: CRONOGRAMA VISUAL
  with tab_cronograma:
        ...
        st.markdown(
            f"""
                <div style="border-left: 4px solid {cor_alerta}; padding-left: 15px; margin-bottom: 20px; background-color: rgba(255,255,255,0.05); padding-top: 10px; padding-bottom: 10px; border-radius: 0 8px 8px 0;">
                    ...
                    <div style="margin-top: 8px; font-size: 12px; font-weight: bold;">📅 Período Executivo: {row['data_inicio'].strftime('%d/%m/%Y') if row['data_inicio'] else 'N/A'} até {row['data_conclusao'].strftime('%d/%m/%Y') if row['data_conclusao'] else 'N/A'}</div>
                </div>
                """,
            unsafe_allow_html=True,
        )

  # ABA 5: CADASTRO COM SALVAMENTO LOCAL AUTOMÁTICO
  with tab_cadastrar:
    ...
    with st.form("form_novo_item_v15", clear_on_submit=True):
      ...
      with c1:
        dimensao_sel = st.selectbox(
            "📌 Dimensão Vinculada (IEG-M)", dimensoes_iegm
        )
      ...
      with c5:
        data_ini = st.date_input(
            "📅 Início da Execução", value=date.today(), format="DD/MM/YYYY"
        )
      ...
      with c6:
        periodo_rep_sel = st.selectbox(
            "🔄 Frequência de Report", periodos_report
        )
        integracao = st.selectbox(
            "🗺️ Integração ao Planejamento",
            ["Sim (PPA/LDO/LOA)", "Parcial", "Não Integrado"],
        )
      with c7:
        forma_execucao = st.text_input("⚙️ Forma de Execução")
        evidencias = st.text_input("📂 Comprovações/Evidências")
        links_evid_sel = st.text_input(
            "🔗 Links das Evidências (Separados por vírgula)"
        )

      status_sel = st.selectbox("🚥 Status Inicial da Demanda", lista_status)
      submit_btn = st.form_submit_button(
          "🚀 Publicar Nova Ação no Plano", use_container_width=True
      )

      if submit_btn:
        if not acao or not responsavel:
          st.error(
              "❌ Os campos 'Título Prático da Ação' e 'Responsável' são"
              " obrigatórios."
          )
        else:
          novo_item = {
              ...
              "links_evidencias": links_evid_sel,
              "status": status_sel,
          }
  ...
  # ABA 6: INTEGRALIZAÇÃO EXTERNA (CSV)
  with tab_import_export:
    ...
    with col_down:
      ...
      st.download_button(
          ...
          file_name="layout_plano_acao.csv",
          mime="text/csv",
          use_container_width=True,
      )
    with col_up:
      ...
      st.download_button(
          label="📦 Exportar Backup de Segurança (.CSV)",
          data=csv_backup.getvalue(),
          file_name="backup_plano_acao.csv",
          mime="text/csv",
          use_container_width=True,
      )
    ...
    uploaded_file = st.file_uploader(
        "Upload em lote para substituição de registros", type=["csv"]
    )
    if uploaded_file is not None:
      try:
        # Tenta carregar testando separadores comuns (\t, ,, ;)
        try:
        ...
        for col_dt in ["data_inicio", "data_conclusao"]:
          if col_dt in df_uploaded.columns:
            df_uploaded[col_dt] = pd.to_datetime(
                df_uploaded[col_dt], errors="coerce"
            ).dt.date

        df_uploaded = df_uploaded.fillna("")

        novos_dados = df_uploaded.to_dict(orient="records")
        st.session_state.plano_acao_db = novos_dados
        salvar_dados_locais()
        st.success("✅ Base de dados atualizada com sucesso via CSV!")
        st.rerun()
      except Exception as e:
