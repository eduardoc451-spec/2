...
# =============================================================================
        ...
        # =============================================================================
        # QUESITO 11.5.1 • ATIVIDADES FISCALIZADAS (Independente - Padrão iGov)
        # =============================================================================
        ...
        # =============================================================================
        # QUESITO 11.6.1 • LICENÇA DA CETESB (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q11_6_1_{ano_sel}", border=True):
            with st.expander("📌 Quesito 11.6.1 - Licença da CETESB", expanded=True):
                ...
                # Recupera os dados salvos no banco
                d1161 = res_data.get("11.6.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentario": ""}
                opc1161 = ["Selecione...", "Sim", "Não"]
                v_salvo_1161 = d1161.get("valor", "Selecione...")
                if v_salvo_1161 not in opc1161:
                    v_salvo_1161 = "Selecione..."
                evidencia_1161_salva = d1161.get("link", "")

                # Definindo chaves do Streamlit
                chave_radio_1161 = f"r1161_in_{ano_sel}"
                chave_link_1161 = f"l1161_in_{ano_sel}"
                chave_coment_1161 = f"coment_11.6.1_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    resp_1161 = st.radio(
                        "Selecione uma opção (11.6.1):",
                        options=opc1161,
                        index=opc1161.index(v_salvo_1161),
                        key=chave_radio_1161
                    )
                ...
                with col2:
                    lk1161 = st.text_area(
                        "Link/Evidência (11.6.1):",
                        value=evidencia_1161_salva,
                        key=chave_link_1161,
                        placeholder="Inserir link da Licença de Operação emitida pela CETESB para a ATT...",
                        height=125
                    )
                    placeholder_links_1161 = st.empty()
                    links_1161_visuais = re.findall(REGEX_PURE_URL, lk1161 or "")
                    if links_1161_visuais:
                        placeholder_links_1161.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_1161_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 11.6.1
                bloco_comentarios("11.6.1", res_data, ano_sel)

                # Feedback visual dinâmico do impacto
                st.markdown(
                    "<span style='color:#6c757d; font-weight:bold;'>📊 Impacto 11.6.1: 0.0 pontos aplicados (Quesito Informativo)</span>",
                    unsafe_allow_html=True
                )
                ...
                # BOTÃO DE SALVAMENTO MANUAL (Padrão iGov)
                # -----------------------------------------------------------------
                if st.button("💾 Salvar Quesito 11.6.1", key=f"btn_salvar_11_6_1_{ano_sel}", type="primary"):
                    lnk_val = lk1161.strip()
                    val_sel = resp_1161
                    comentario_para_salvar = st.session_state.get(chave_coment_1161, d1161.get("comentario", ""))
                    ...
                    # Verificação de novos links para disparo do modal de validação
                    links_atuais = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, lnk_val or "")]
                    links_antigos = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, evidencia_1161_salva or "")]

                    if lnk_val != evidencia_1161_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_11_6_1_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_11_6_1_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 11.6.1 salvos com sucesso!", icon="✅")
                    st.rerun()

        # GATILHO DO MODAL 11.6.1 (Fora do container principal)
        ...
        # =============================================================================
        # QUESITO 11.6.1.1 • PRAZO DE VALIDADE DA LICENÇA (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q11_6_1_1_{ano_sel}", border=True):
            with st.expander("📌 Quesito 11.6.1.1 - Prazo de Validade", expanded=True):
                ...
                # Recupera os dados salvos no banco
                d11611 = res_data.get("11.6.1.1") or {"valor": "31/12/2024", "pontos": 0.0, "link": "", "comentario": ""}
                v_salvo_11611 = d11611.get("valor", "31/12/2024")
                evidencia_11611_salva = d11611.get("link", "")

                # Extração e validação segura da data salva (Dia/Mês/Ano)
                try:
                    dia_salvo, mes_salvo, ano_salvo = map(int, str(v_salvo_11611).split("/"))
                except Exception:
                    dia_salvo, mes_salvo, ano_salvo = 31, 12, 2024

                # Definindo chaves do Streamlit
                chave_dia_11611 = f"q11611_d_{ano_sel}"
                chave_mes_11611 = f"q11611_m_{ano_sel}"
                chave_ano_11611 = f"q11611_a_{ano_sel}"
                chave_link_11611 = f"l11611_in_{ano_sel}"
                chave_coment_11611 = f"coment_11.6.1.1_{ano_sel}"
                ...
                with col2:
                    lk11611 = st.text_area(
                        "Link/Evidência (11.6.1.1):",
                        value=evidencia_11611_salva,
                        key=chave_link_11611,
                        placeholder="Inserir link do documento que comprove o prazo de validade da licença...",
                        height=125
                    )
                    placeholder_links_11611 = st.empty()
                    links_11611_visuais = re.findall(REGEX_PURE_URL, lk11611 or "")
                    if links_11611_visuais:
                        placeholder_links_11611.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_11611_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 11.6.1.1
                bloco_comentarios("11.6.1.1", res_data, ano_sel)

                # Feedback visual dinâmico do impacto
                st.markdown(
                    "<span style='color:#6c757d; font-weight:bold;'>📊 Impacto 11.6.1.1: 0.0 pontos aplicados (Quesito Informativo)</span>",
                    unsafe_allow_html=True
                )
                ...
                # BOTÃO DE SALVAMENTO MANUAL (Padrão iGov)
                # -----------------------------------------------------------------
                if st.button("💾 Salvar Quesito 11.6.1.1", key=f"btn_salvar_11_6_1_1_{ano_sel}", type="primary"):
                    lnk_val = lk11611.strip()
                    val_data_formatada = f"{dia_val:02d}/{mes_val:02d}/{ano_val}"
                    comentario_para_salvar = st.session_state.get(chave_coment_11611, d11611.get("comentario", ""))

                    # Persistência no banco via save_resp (sempre 0.0 pontos)
                    save_resp(
                        qid="11.6.1.1",
                        valor=val_data_formatada,
                        pontos=0.0,
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    # Verificação de novos links para disparo do modal de validação
                    links_atuais = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, lnk_val or "")]
                    links_antigos = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, evidencia_11611_salva or "")]

                    if lnk_val != evidencia_11611_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_11_6_1_1_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_11_6_1_1_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 11.6.1.1 salvos com sucesso!", icon="✅")
                    st.rerun()

        # GATILHO DO MODAL 11.6.1.1 (Fora do container principal)
        ...
        # =============================================================================
        # QUESITO 12.1 • FORMAS DE PROCESSAMENTO REALIZADAS (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q12_1_{ano_sel}", border=True):
            with st.expander("📌 Quesito 12.1 - Formas de Processamento", expanded=True):
                ...
                d121 = res_data.get("12.1") or {"valor": "[]", "pontos": 0.0, "link": "", "comentario": ""}
                v_salvo_121 = d121.get("valor", "[]")
                pts_salvos_121 = float(d121.get("pontos", 0.0))
                evidencia_121_salva = d121.get("link", "")

                # Converte o valor salvo de string para lista
                try:
                    import ast
                    lista_salva_121 = ast.literal_eval(v_salvo_121) if isinstance(v_salvo_121, str) and v_salvo_121.startswith("[") else []
                except Exception:
                    lista_salva_121 = []
                ...
                # Definindo chaves do Streamlit
                chave_link_121 = f"l121_in_{ano_sel}"
                chave_coment_121 = f"coment_12.1_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    # Renderização das opções de múltipla escolha (Checkboxes)
                    for idx, (opt, p_val) in enumerate(opts121.items()):
                        st.checkbox(
                            opt,
                            value=(opt in lista_salva_121 or opt in str(v_salvo_121)),
                            key=f"ck_121_{idx}_{ano_sel}"
                        )

                    # Exibição do impacto da pontuação salva
                    st.metric(label="Impacto na Pontuação", value=f"{pts_salvos_121:.1f} pts")
                ...
                with col2:
                    lk121 = st.text_area(
                        ...
                        key=chave_link_121,
                        placeholder="Inserir link da documentação referente às formas de processamento...",
                        height=150
                    )
                    placeholder_links_121 = st.empty()
                    links_121_visuais = re.findall(REGEX_PURE_URL, lk121 or "")
                    if links_121_visuais:
                        placeholder_links_121.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_121_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 12.1
                bloco_comentarios("12.1", res_data, ano_sel)

                # Feedback visual do impacto salvo
                st.markdown(
                    f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto 12.1: {pts_salvos_121:.1f} pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                if st.button("💾 Salvar Quesito 12.1", key=f"btn_salvar_12_1_{ano_sel}", type="primary"):
                    # Calcula as opções marcadas e a pontuação total
                    selecionados_121 = []
                    pts_calculados_121 = 0.0

                    for idx, (opt, p_val) in enumerate(opts121.items()):
                        if st.session_state.get(f"ck_121_{idx}_{ano_sel}", False):
                            selecionados_121.append(opt)
                            pts_calculados_121 += p_val

                    lnk_val = lk121.strip()
                    val_str_121 = str(selecionados_121)
                    ...
                    save_resp(
                        qid="12.1",
                        valor=val_str_121,
                        pontos=float(pts_calculados_121),
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 12.1 salvos com sucesso!", icon="✅")
                    st.rerun()
        ...
        # =============================================================================
        # QUESITO 13.0 • EXISTÊNCIA DE ATERRO (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q13_0_{ano_sel}", border=True):
            with st.expander("📌 Quesito 13.0 - Existência de Aterro", expanded=True):
                ...
                if v_salvo_130 not in opc130:
                    v_salvo_130 = "Selecione..."
                
                evidencia_130_salva = d130.get("link", "")

                # Definindo chaves do Streamlit
                chave_resposta_130 = f"r130_in_{ano_sel}"
                chave_link_130 = f"l130_in_{ano_sel}"
                chave_coment_130 = f"coment_13.0_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    v_selecionado_130 = st.radio(
                        "Selecione uma opção (13.0):",
                        options=opc130,
                        index=opc130.index(v_salvo_130),
                        key=chave_resposta_130
                    )
                ...
                with col2:
                    lk130 = st.text_area(
                        ...
                        key=chave_link_130,
                        placeholder="Inserir link da documentação referente ao aterro...",
                        height=125
                    )
                    placeholder_links_130 = st.empty()
                    links_130_visuais = re.findall(REGEX_PURE_URL, lk130 or "")
                    if links_130_visuais:
                        placeholder_links_130.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_130_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 13.0
                bloco_comentarios("13.0", res_data, ano_sel)

                # Feedback visual do impacto
                st.markdown(
                    "<span style='color:#28a745; font-weight:bold;'>📊 Impacto 13.0: 0.0 pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                if st.button("💾 Salvar Quesito 13.0", key=f"btn_salvar_13_0_{ano_sel}", type="primary"):
                    ...
                    # Regra de Limpeza Condicional Lógica:
                    # Se respondeu "Não", reseta a resposta do quesito dependente (13.1) no banco e memória
                    if v_selecionado_130 == "Não":
                        coment_131 = res_data.get("13.1", {}).get("comentario", "")
                        save_resp(
                            qid="13.1",
                            valor="[]",
                            pontos=-110.0,
                            link="",
                            comentario=coment_131
                        )
                        res_data["13.1"] = {
                            "valor": "[]",
                            "pontos": -110.0,
                    ...
                    if lnk_val != evidencia_130_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_13_0_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_13_0_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 13.0 salvos com sucesso!", icon="✅")
                    st.rerun()
        ...
        # =============================================================================
        # QUESITO 14.2 • LOCALIZAÇÃO DO DESCARTE IRREGULAR (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q14_2_{ano_sel}", border=True):
            with st.expander("📌 Quesito 14.2 - Endereço dos Locais", expanded=True):
                ...
                pts_salvos_142 = float(d142.get("pontos", 0.0))
                evidencia_142_salva = d142.get("link", "")

                # Definindo chaves do Streamlit
                chave_resposta_142 = f"t142_in_{ano_sel}"
                chave_link_142 = f"l142_in_{ano_sel}"
                chave_coment_142 = f"coment_14.2_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    v_input_142 = st.text_area(
                        "Endereço dos locais (14.2):",
                        value=v_salvo_142,
                        key=chave_resposta_142,
                        placeholder="Informe os endereços, coordenadas ou pontos de referência dos locais...",
                        height=125
                    )

                    # Exibição de métrica de impacto salva
                    st.metric(label="Impacto na Pontuação (Salvo)", value=f"{pts_salvos_142:.1f} pts")
                ...
                with col2:
                    lk142 = st.text_area(
                        ...
                        key=chave_link_142,
                        placeholder="Inserir link com mapeamento, fotos georreferenciadas ou documentos...",
                        height=125
                    )
                    placeholder_links_142 = st.empty()
                    links_142_visuais = re.findall(REGEX_PURE_URL, lk142 or "")
                    if links_142_visuais:
                        placeholder_links_142.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_142_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 14.2
                bloco_comentarios("14.2", res_data, ano_sel)

                # Feedback visual dinâmico do impacto salvo
                st.markdown(
                    f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto 14.2: {pts_salvos_142:.1f} pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                if st.button("💾 Salvar Quesito 14.2", key=f"btn_salvar_14_2_{ano_sel}", type="primary"):
                    val_texto_142 = v_input_142.strip()
                    lnk_val = lk142.strip()
                    pts_calculados_142 = 0.0
                    ...
                    save_resp(
                        qid="14.2",
                        valor=val_texto_142,
                        pontos=float(pts_calculados_142),
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    if lnk_val != evidencia_142_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_14_2_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_14_2_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 14.2 salvos com sucesso!", icon="✅")
                    st.rerun()
        ...
        # =============================================================================
        # QUESITO 15.1.1 • REGULAÇÃO DE ÁGUA POTÁVEL (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q15_1_1_{ano_sel}", border=True):
            with st.expander("📌 Quesito 15.1.1 - Regulação de Água Potável", expanded=True):
                ...
                # Recuperação dos dados salvos no banco
                d1511 = res_data.get("15.1.1") or {"valor": "", "pontos": 0.0, "link": "", "comentario": ""}
                v_salvo_1511 = d1511.get("valor", "")
                pts_salvos_1511 = float(d1511.get("pontos", 0.0))
                evidencia_1511_salva = d1511.get("link", "")

                # Definindo chaves do Streamlit
                chave_texto_1511 = f"t1511_in_{ano_sel}"
                chave_link_1511 = f"l1511_in_{ano_sel}"
                chave_coment_1511 = f"coment_15.1.1_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    txt_entidade_1511 = st.text_area(
                        "Entidade responsável (15.1.1):",
                        value=v_salvo_1511,
                        key=chave_texto_1511,
                    ...
                    st.metric(label="Impacto na Pontuação (Salvo)", value=f"{pts_salvos_1511:.1f} pts")

                with col2:
                    lk1511 = st.text_area(
                        "Link/Evidência (15.1.1):",
                        value=evidencia_1511_salva,
                        key=chave_link_1511,
                        placeholder="Inserir link da lei de criação, convênio ou contrato de regulação de água...",
                        height=125
                    )
                    placeholder_links_1511 = st.empty()
                    links_1511_visuais = re.findall(REGEX_PURE_URL, lk1511 or "")
                    if links_1511_visuais:
                        placeholder_links_1511.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_1511_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 15.1.1
                bloco_comentarios("15.1.1", res_data, ano_sel)

                # Feedback visual dinâmico do impacto salvo
                st.markdown(
                    f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto 15.1.1: {pts_salvos_1511:.1f} pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                # BOTÃO DE SALVAMENTO MANUAL (Padrão iGov)
                # -----------------------------------------------------------------
                if st.button("💾 Salvar Quesito 15.1.1", key=f"btn_salvar_15_1_1_{ano_sel}", type="primary"):
                    val_entidade_1511 = txt_entidade_1511.strip()
                    lnk_val = lk1511.strip()
                    pts_calculados_1511 = 0.0
                    comentario_para_salvar = st.session_state.get(chave_coment_1511, d1511.get("comentario", ""))

                    # Persistência no banco de dados
                    save_resp(
                        qid="15.1.1",
                        valor=val_entidade_1511,
                        pontos=float(pts_calculados_1511),
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    # Verificação de novos links para disparo do modal de validação
                    links_atuais = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, lnk_val or "")]
                    links_antigos = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, evidencia_1511_salva or "")]

                    if lnk_val != evidencia_1511_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_15_1_1_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_15_1_1_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 15.1.1 salvos com sucesso!", icon="✅")
                    st.rerun()

        # GATILHO DO MODAL 15.1.1 (Fora do container principal)
        if st.session_state.get(f"gatilho_modal_15_1_1_{ano_sel}", False):
            if "modal_aviso_link" in globals():
        ...
        # =============================================================================
        # QUESITO 15.1.3 • REGULAÇÃO DE RESÍDUOS SÓLIDOS (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q15_1_3_{ano_sel}", border=True):
            with st.expander("📌 Quesito 15.1.3 - Regulação de Resíduos Sólidos", expanded=True):
                ...
                # Recuperação dos dados salvos no banco
                d1513 = res_data.get("15.1.3") or {"valor": "", "pontos": 0.0, "link": "", "comentario": ""}
                v_salvo_1513 = d1513.get("valor", "")
                pts_salvos_1513 = float(d1513.get("pontos", 0.0))
                evidencia_1513_salva = d1513.get("link", "")

                # Definindo chaves do Streamlit
                chave_texto_1513 = f"t1513_in_{ano_sel}"
                chave_link_1513 = f"l1513_in_{ano_sel}"
                chave_coment_1513 = f"coment_15.1.3_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    txt_entidade_1513 = st.text_area(
                        "Entidade responsável (15.1.3):",
                        value=v_salvo_1513,
                        key=chave_texto_1513,
                        placeholder="Nome ou sigla da entidade responsável (ex: ARSESP, ARSAE, Agência Municipal...)",
                        height=125
                    )

                    # Exibição da métrica de impacto (fixo em 0.0 pts por ser informativo)
                    st.metric(label="Impacto na Pontuação (Salvo)", value=f"{pts_salvos_1513:.1f} pts")

                with col2:
                    lk1513 = st.text_area(
                        "Link/Evidência (15.1.3):",
                        value=evidencia_1513_salva,
                        key=chave_link_1513,
                        placeholder="Inserir link da lei de criação, convênio ou contrato de regulação de limpeza e resíduos...",
                        height=125
                    )
                    placeholder_links_1513 = st.empty()
                    links_1513_visuais = re.findall(REGEX_PURE_URL, lk1513 or "")
                    if links_1513_visuais:
                        placeholder_links_1513.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_1513_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 15.1.3
                bloco_comentarios("15.1.3", res_data, ano_sel)

                # Feedback visual dinâmico do impacto salvo
                st.markdown(
                    f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto 15.1.3: {pts_salvos_1513:.1f} pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                # BOTÃO DE SALVAMENTO MANUAL (Padrão iGov)
                # -----------------------------------------------------------------
                if st.button("💾 Salvar Quesito 15.1.3", key=f"btn_salvar_15_1_3_{ano_sel}", type="primary"):
                    val_entidade_1513 = txt_entidade_1513.strip()
                    lnk_val = lk1513.strip()
                    pts_calculados_1513 = 0.0
                    comentario_para_salvar = st.session_state.get(chave_coment_1513, d1513.get("comentario", ""))

                    # Persistência no banco de dados
                    save_resp(
                        qid="15.1.3",
                        valor=val_entidade_1513,
                        pontos=float(pts_calculados_1513),
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    # Verificação de novos links para disparo do modal de validação
                    links_atuais = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, lnk_val or "")]
                    links_antigos = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, evidencia_1513_salva or "")]

                    if lnk_val != evidencia_1513_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_15_1_3_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_15_1_3_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 15.1.3 salvos com sucesso!", icon="✅")
                    st.rerun()

        # GATILHO DO MODAL 15.1.3 (Fora do container principal)
        if st.session_state.get(f"gatilho_modal_15_1_3_{ano_sel}", False):
            if "modal_aviso_link" in globals():
                modal_aviso_link("15.1.3", st.session_state.get(f"links_pendentes_15_1_3_{ano_sel}", []))
            st.session_state[f"gatilho_modal_15_1_3_{ano_sel}"] = False

        # =============================================================================
        # QUESITO 15.1.4 • REGULAÇÃO DE DRENAGEM E ÁGUAS PLUVIAIS (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_q15_1_4_{ano_sel}", border=True):
            with st.expander("📌 Quesito 15.1.4 - Regulação de Águas Pluviais", expanded=True):
                ...
                # Recuperação dos dados salvos no banco
                d1514 = res_data.get("15.1.4") or {"valor": "", "pontos": 0.0, "link": "", "comentario": ""}
                v_salvo_1514 = d1514.get("valor", "")
                pts_salvos_1514 = float(d1514.get("pontos", 0.0))
                evidencia_1514_salva = d1514.get("link", "")

                # Definindo chaves do Streamlit
                chave_texto_1514 = f"t1514_in_{ano_sel}"
                chave_link_1514 = f"l1514_in_{ano_sel}"
                chave_coment_1514 = f"coment_15.1.4_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    txt_entidade_1514 = st.text_area(
                        "Entidade responsável (15.1.4):",
                        value=v_salvo_1514,
                        key=chave_texto_1514,
                        placeholder="Nome ou sigla da entidade responsável (ex: ARSESP, ARSAE, Agência Municipal...)",
                        height=125
                    )

                    # Exibição da métrica de impacto (fixo em 0.0 pts por ser informativo)
                    st.metric(label="Impacto na Pontuação (Salvo)", value=f"{pts_salvos_1514:.1f} pts")

                with col2:
                    lk1514 = st.text_area(
                        "Link/Evidência (15.1.4):",
                        value=evidencia_1514_salva,
                        key=chave_link_1514,
                        placeholder="Inserir link da lei de criação, convênio ou contrato de regulação de drenagem e águas pluviais...",
                        height=125
                    )
                    placeholder_links_1514 = st.empty()
                    links_1514_visuais = re.findall(REGEX_PURE_URL, lk1514 or "")
                    if links_1514_visuais:
                        placeholder_links_1514.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_1514_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 15.1.4
                bloco_comentarios("15.1.4", res_data, ano_sel)

                # Feedback visual dinâmico do impacto salvo
                st.markdown(
                    f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto 15.1.4: {pts_salvos_1514:.1f} pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                # BOTÃO DE SALVAMENTO MANUAL (Padrão iGov)
                # -----------------------------------------------------------------
                if st.button("💾 Salvar Quesito 15.1.4", key=f"btn_salvar_15_1_4_{ano_sel}", type="primary"):
                    val_entidade_1514 = txt_entidade_1514.strip()
                    lnk_val = lk1514.strip()
                    pts_calculados_1514 = 0.0
                    comentario_para_salvar = st.session_state.get(chave_coment_1514, d1514.get("comentario", ""))

                    # Persistência no banco de dados
                    save_resp(
                        qid="15.1.4",
                        valor=val_entidade_1514,
                        pontos=float(pts_calculados_1514),
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    # Verificação de novos links para disparo do modal de validação
                    links_atuais = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, lnk_val or "")]
                    links_antigos = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, evidencia_1514_salva or "")]

                    if lnk_val != evidencia_1514_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_15_1_4_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_15_1_4_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Resposta e comentários do Quesito 15.1.4 salvos com sucesso!", icon="✅")
                    st.rerun()

        # GATILHO DO MODAL 15.1.4 (Fora do container principal)
        if st.session_state.get(f"gatilho_modal_15_1_4_{ano_sel}", False):
            if "modal_aviso_link" in globals():
        ...
        # =============================================================================
        # QUESITO 16.0 • IMPRESSÕES FINAIS (Padrão iGov)
        # =============================================================================
        st.divider()
        ...
        with st.container(key=f"bloco_isolado_q16_0_{ano_sel}", border=True):
            with st.expander("📌 Quesito 16.0 - Impressões Finais", expanded=True):
                ...
                # Recuperação segura (suporta '16.0' e a chave legada '16.0_valor')
                d160 = res_data.get("16.0") or res_data.get("16.0_valor") or {"valor": "", "pontos": 0.0, "link": "", "comentario": ""}
                v_salvo_160 = d160.get("valor", "")
                pts_salvos_160 = float(d160.get("pontos", 0.0))
                evidencia_160_salva = d160.get("link", "")

                # Chaves explícitas do Streamlit Session State
                chave_texto_160 = f"t160_in_{ano_sel}"
                chave_link_160 = f"l160_in_{ano_sel}"
                chave_coment_160 = f"coment_16.0_{ano_sel}"

                col1, col2 = st.columns([1, 1])

                with col1:
                    txt_impressoes_160 = st.text_area(
                        "Impressões Finais (16.0):",
                        value=v_salvo_160,
                        key=chave_texto_160,
                        placeholder="Escreva aqui suas considerações gerais, facilidades, dificuldades ou sugestões quanto ao processo...",
                        height=150
                    )

                    # Exibição da métrica de impacto (fixo em 0.0 pts por ser informativo)
                    st.metric(label="Impacto na Pontuação (Salvo)", value=f"{pts_salvos_160:.1f} pts")
                ...
                with col2:
                    lk160 = st.text_area(
                        ...
                        key=chave_link_160,
                        placeholder="Inserir link para relatórios complementares ou documentos adicionais (se houver)...",
                        height=150
                    )
                    placeholder_links_160 = st.empty()
                    links_160_visuais = re.findall(REGEX_PURE_URL, lk160 or "")
                    if links_160_visuais:
                        placeholder_links_160.markdown(
                            "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_160_visuais])
                        )

                # Renderiza o bloco de comentários do Quesito 16.0
                bloco_comentarios("16.0", res_data, ano_sel)

                # Feedback visual dinâmico do impacto salvo
                st.markdown(
                    f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto 16.0: {pts_salvos_160:.1f} pontos aplicados</span>",
                    unsafe_allow_html=True
                )
                ...
                if st.button("💾 Salvar Quesito 16.0", key=f"btn_salvar_16_0_{ano_sel}", type="primary"):
                    val_impressoes_160 = txt_impressoes_160.strip()
                    lnk_val = lk160.strip()
                    pts_calculados_160 = 0.0
                    comentario_para_salvar = st.session_state.get(chave_coment_160, d160.get("comentario", ""))

                    # Persistência no banco de dados
                    save_resp(
                        qid="16.0",
                        valor=val_impressoes_160,
                        pontos=float(pts_calculados_160),
                        link=lnk_val,
                        comentario=comentario_para_salvar
                    )
                    ...
                    if lnk_val != evidencia_160_salva and links_atuais and links_atuais != links_antigos:
                        st.session_state[f"links_pendentes_16_0_{ano_sel}"] = links_atuais
                        st.session_state[f"gatilho_modal_16_0_{ano_sel}"] = True

                    st.cache_data.clear()
                    st.toast("Considerações finais do Quesito 16.0 salvas com sucesso!", icon="✅")
                    st.rerun()
        ...
        # =============================================================================
        # QUESITO A4.1 • DADOS DO SINISA (Padrão iGov)
        # =============================================================================
        with st.container(key=f"bloco_isolado_ext_a41_{ano_sel}", border=True):
            st.subheader("QUESITO A4.1 • Envio de Dados ao SINISA")
            st.write("**Informe quais dados foram enviados ao SINISA:**")
            st.caption("Dados do SINISA (Sistema Nacional de Informações sobre Saneamento Básico). Não sujeitos à validação.")

            # Recuperação dos dados salvos no banco
            dA41 = res_data.get("A4.1") or {"valor": "[]", "pontos": 0.0, "link": "", "comentario": ""}
            v_salvo_A41 = str(dA41.get("valor", "[]"))
            pts_salvos_A41 = float(dA41.get("pontos", 0.0))
            evidencia_A41_salva = dA41.get("link", "")

            # Chaves do Streamlit Session State
            chave_link_A41 = f"lA41_evid_unique_{ano_sel}"
            chave_coment_A41 = f"coment_A4.1_{ano_sel}"
            ...
            with col1:
                selA41 = []
                st.write("**Serviços Informados:**")
                for i, txt in enumerate(optsA41):
                    # Se for a primeira vez (banco contendo "[]"), inicia como True por padrão
                    valor_inicial = txt in v_salvo_A41 if v_salvo_A41 != "[]" else True
                    
                    if st.checkbox(txt, value=valor_inicial, key=f"qA41_opt_{i}_{ano_sel}"):
                        selA41.append(txt)

                # Métrica de impacto (fixo em 0.0 pts por ser dado do SINISA)
                st.metric(label="Impacto na Pontuação (Salvo)", value=f"{pts_salvos_A41:.1f} pts")

            with col2:
                lA41 = st.text_area(
                    "Link/Evidência (A4.1):",
                    value=evidencia_A41_salva,
                    key=chave_link_A41,
                    placeholder="Inserir link do comprovante de envio ou certidão do SINISA...",
                    height=140
                )
                placeholder_links_A41 = st.empty()
                links_A41_visuais = re.findall(REGEX_PURE_URL, lA41 or "")
                if links_A41_visuais:
                    placeholder_links_A41.markdown(
                        "**🔗 Link ativo:** " + " | ".join([f"[{u[0] if isinstance(u, tuple) else u}]({u[0] if isinstance(u, tuple) else u})" for u in links_A41_visuais])
                    )

            # Renderiza o bloco de comentários do Quesito A4.1
            bloco_comentarios("A4.1", res_data, ano_sel)

            # Feedback visual dinâmico
            st.markdown(
                f"<span style='color:#28a745; font-weight:bold;'>📊 Impacto A4.1: {pts_salvos_A41:.1f} pontos aplicados</span>",
                unsafe_allow_html=True
            )
            ...
            # BOTÃO DE SALVAMENTO MANUAL (Padrão iGov)
            # -----------------------------------------------------------------
            if st.button("💾 Salvar Quesito A4.1", key=f"btn_salvar_ext_a41_{ano_sel}", type="primary"):
                val_a41_final = str(selA41)
                lnk_val = lA41.strip()
                pts_calculados_a41 = 0.0
                comentario_para_salvar = st.session_state.get(chave_coment_A41, dA41.get("comentario", ""))

                # Persistência no banco de dados
                save_resp(
                    qid="A4.1",
                    valor=val_a41_final,
                    pontos=pts_calculados_a41,
                    link=lnk_val,
                    comentario=comentario_para_salvar
                )
                ...
                # Verificação de novos links para disparo do modal de validação
                links_atuais = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, lnk_val or "")]
                links_antigos = [u[0] if isinstance(u, tuple) else u for u in re.findall(REGEX_PURE_URL, evidencia_A41_salva or "")]

                if lnk_val != evidencia_A41_salva and links_atuais and links_atuais != links_antigos:
                    st.session_state[f"links_pendentes_A4_1_{ano_sel}"] = links_atuais
                    st.session_state[f"gatilho_modal_A4_1_{ano_sel}"] = True

                st.cache_data.clear()
                st.toast("Dados do Quesito A4.1 (SINISA) salvos com sucesso!", icon="✅")
                st.rerun()

        # GATILHO DO MODAL A4.1 (Fora do container principal)
        if st.session_state.get(f"gatilho_modal_A4_1_{ano_sel}", False):
            if "modal_aviso_link" in globals():
