...
# =============================================================================
        ...
        with st.container(key=f"container_bloco_ouvidorsus_34_0_{ano_sel}", border=False):
            with st.expander("📌 Quesito 34.0 - Uso do Sistema OuvidorSUS ou Equivalente", expanded=True):
                ...
                st.caption("ℹ️ *Selecione a opção desejada, insira o link/evidência e clique no botão de salvar para registrar os dados.*")
                ...
                # Form para agrupar as interações de escolha e texto sem recarregamento imediato
                with st.form(key=f"form_quesito_34_0_{ano_sel}"):
                    ...
                    with c340_2:
                        link_34_0 = st.text_area(
                            "Link/Evidência (34.0):", 
                            value=d34_0.get("link", ""), 
                            key=f"txt_34_0_{ano_sel}", 
                            height=110
                        )
                        
                        # Extração e Separação de Mídias e Links
                        links_34_0_atuais = re.findall(r'(https?://[^\s]+)', link_34_0)
                        midias_34_0 = [u for u in links_34_0_atuais if re.search(r'\.(png|jpg|jpeg|gif|webp|pdf)$', u, re.IGNORECASE)]
                        links_simples_34_0 = [u for u in links_34_0_atuais if u not in midias_34_0]

                        if links_34_0_atuais:
                            with st.expander("📎 Mídias e Links Anexados (34.0)", expanded=False):
                                if links_simples_34_0:
                                    botoes_34_0 = " | ".join([f"🔗 [{u}]({u})" for u in links_simples_34_0])
                                    st.markdown(f"**Links Diretos:** {botoes_34_0}")
                                
                                for midia in midias_34_0:
                                    if re.search(r'\.(png|jpg|jpeg|gif|webp)$', midia, re.IGNORECASE):
                                        st.image(midia, caption="Evidência Visual (34.0)", use_container_width=True)
                                    elif midia.lower().endswith('.pdf'):
                                        st.markdown(f"📄 **Documento PDF:** [{midia}]({midia})")
                    ...
                    if sel_34_0 == "Selecione...":
                        st.markdown("⚠️ **Pontuação:** `0.0 pontos` (Por favor, selecione uma opção válida)")
                    elif pts_34_0 > 0:
                        st.success(f"✅ Pontuação Atribuída no Quesito 34.0: `{pts_34_0:.1f} pontos`.")
                    else:
                        st.markdown(f"📊 **Pontuação Atribuída no Quesito 34.0:** `{pts_34_0:.1f} pontos`")

                    # Botão Único para Submissão e Salvamento
                    btn_salvar_34_0 = st.form_submit_button("💾 Salvar Resposta (34.0)", type="primary")

                if btn_salvar_34_0:
                    save_resp("34.0", sel_34_0, pts_34_0, link_34_0)
                    res_data["34.0"] = {"valor": sel_34_0, "pontos": pts_34_0, "link": link_34_0}
                    st.toast("Resposta da Seção 34.0 salva com sucesso!", icon="✅")
                    st.rerun()
        ...
        with st.container(key=f"container_bloco_sna_35_0_{ano_sel}", border=False):
            with st.expander("📌 Quesito 35.0 - Existência do Componente Municipal do SNA", expanded=True):
                ...
                with st.form(key=f"form_quesito_35_0_{ano_sel}"):
                    ...
                    with c350_2:
                        link_35_0 = st.text_area(
                            "Link/Evidência (35.0):", 
                            value=d35_0.get("link", ""), 
                            key=f"txt_35_0_{ano_sel}", 
                            height=110
                        )
                        
                        # Extração e Separação de Mídias e Links
                        links_35_0_atuais = re.findall(r'(https?://[^\s]+)', link_35_0)
                        midias_35_0 = [u for u in links_35_0_atuais if re.search(r'\.(png|jpg|jpeg|gif|webp|pdf)$', u, re.IGNORECASE)]
                        links_simples_35_0 = [u for u in links_35_0_atuais if u not in midias_35_0]

                        if links_35_0_atuais:
                            with st.expander("📎 Mídias e Links Anexados (35.0)", expanded=False):
                                if links_simples_35_0:
                                    botoes_35_0 = " | ".join([f"🔗 [{u}]({u})" for u in links_simples_35_0])
                                    st.markdown(f"**Links Diretos:** {botoes_35_0}")
                                
                                for midia in midias_35_0:
                                    if re.search(r'\.(png|jpg|jpeg|gif|webp)$', midia, re.IGNORECASE):
                                        st.image(midia, caption="Evidência Visual (35.0)", use_container_width=True)
                                    elif midia.lower().endswith('.pdf'):
                                        st.markdown(f"📄 **Documento PDF:** [{midia}]({midia})")
                    ...
                    if sel_35_0 == "Selecione...":
                        ...
                        st.markdown(f"📊 **Pontuação Aplicada no Quesito 35.0:** `{pts_35_0:.1f} pontos` (Dados Informativos)")

                    btn_salvar_35_0 = st.form_submit_button("💾 Salvar Resposta (35.0)", type="primary")

                if btn_salvar_35_0:
                    save_resp("35.0", sel_35_0, pts_35_0, link_35_0)
                    res_data["35.0"] = {"valor": sel_35_0, "pontos": pts_35_0, "link": link_35_0}
                    st.toast("Resposta da Seção 35.0 salva com sucesso!", icon="✅")
                    st.rerun()
        ...
        with st.container(key=f"container_bloco_caracteristicas_sna_35_1_{ano_sel}", border=False):
            with st.expander("📌 Quesito 35.1 - Características do Componente SNA", expanded=True):
                ...
                st.caption("ℹ️ *Marque as caixas correspondentes, informe o link de evidência e salve os dados.*")
                ...
                with st.form(key=f"form_quesito_35_1_{ano_sel}"):
                    ...
                    with c351_2:
                        ...
                        link_35_1 = st.text_area(
                            "Link/Evidência (35.1):", 
                            value=d35_1.get("link", ""), 
                            key=f"txt_35_1_{ano_sel}", 
                            height=110
                        )
                        
                        # Extração e Separação de Mídias e Links
                        links_35_1_atuais = re.findall(r'(https?://[^\s]+)', link_35_1)
                        midias_35_1 = [u for u in links_35_1_atuais if re.search(r'\.(png|jpg|jpeg|gif|webp|pdf)$', u, re.IGNORECASE)]
                        links_simples_35_1 = [u for u in links_35_1_atuais if u not in midias_35_1]

                        if links_35_1_atuais:
                            with st.expander("📎 Mídias e Links Anexados (35.1)", expanded=False):
                                if links_simples_35_1:
                                    botoes_35_1 = " | ".join([f"🔗 [{u}]({u})" for u in links_simples_35_1])
                                    st.markdown(f"**Links Diretos:** {botoes_35_1}")
                                
                                for midia in midias_35_1:
                                    if re.search(r'\.(png|jpg|jpeg|gif|webp)$', midia, re.IGNORECASE):
                                        st.image(midia, caption="Evidência Visual (35.1)", use_container_width=True)
                                    elif midia.lower().endswith('.pdf'):
                                        st.markdown(f"📄 **Documento PDF:** [{midia}]({midia})")
                    ...
                    if pts_totais_35_1 > 0:
                        st.markdown(f"📊 **Pontuação Total Acumulada no Quesito 35.1:** :green[+{pts_totais_35_1:.1f} pontos]")
                    else:
                        st.markdown(f"📊 **Pontuação Total Acumulada no Quesito 35.1:** `{pts_totais_35_1:.1f} pontos`")

                    btn_salvar_35_1 = st.form_submit_button("💾 Salvar Resposta (35.1)", type="primary")

                if btn_salvar_35_1:
                    save_resp("35.1", str_35_1, pts_totais_35_1, link_35_1)
                    res_data["35.1"] = {"valor": str_35_1, "pontos": pts_totais_35_1, "link": link_35_1}
                    st.toast("Resposta da Seção 35.1 salva com sucesso!", icon="✅")
                    st.rerun()
