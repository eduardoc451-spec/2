...
def renderizar_questao(qid, res_data):
    """Renderiza a questão mantendo a integridade do histórico de comentários."""
    dados_q = res_data.get(qid, {})
...
def mostrar_formulario_cidade():
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_2_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 2.0 - Treinamento e Capacitação de Agentes", expanded=True):
            ...
            # Estado inicial / persistente
            d20 = res_data.get("2.0") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 2.0
            if st.button("💾 Salvar Quesito 2.0", key=f"btn_salvar_q20_{ano_sel}", type="primary"):
                pts_calc_20 = opcoes_20.get(val_radio_20, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados para NÃO sobrescrever
                coments_atuais_20 = res_data.get("2.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_20,
                    link=link_20,
                    comentarios=coments_atuais_20
                )

                # ATUALIZA A MEMÓRIA LOCAL: Garante atualização em tempo real sem rerun
                if "2.0" not in res_data:
                    res_data["2.0"] = {}
                res_data["2.0"]["pontos"] = pts_calc_20
                res_data["2.0"]["valor"] = val_radio_20
                res_data["2.0"]["link"] = link_20

                st.toast("Quesito 2.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links (caso o modal esteja implementado)
                links_encontrados_20 = re.findall(r'https?://[^\s]+', link_20 or "")
                if links_encontrados_20 and 'modal_aviso_link' in globals():
                    modal_aviso_link("2.0", links_encontrados_20)

            # Exibição dinamicamente atualizada da pontuação
            d20_atualizado = res_data.get("2.0", {})
            pts_atuais_20 = d20_atualizado.get("pontos", 0.0)
            val_atual_20 = d20_atualizado.get("valor", "Selecione...")

            cor_txt_20 = "#28a745" if pts_atuais_20 == 20.0 else ("#dc3545" if val_atual_20 != "Selecione..." else "#6c757d")
            st.markdown(
    ...
    # Exposição da variável r20 para dependências condicionais de outros quesitos
    r20 = res_data.get("2.0", {}).get("valor", v_salvo_20)

    # =============================================================================
    # QUESITO 2.1 • DATA DA ÚLTIMA CAPACITAÇÃO
    # =============================================================================
    with st.container(key=f"container_bloco_compdec_2_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 2.1 - Data da Última Capacitação de Agentes", expanded=True):
            ...
            * 🚫 **Capacitações em {ano_sel + 1}:** Não pontuam (00 pontos).
            ...
            # Estado inicial / persistente
            d21 = res_data.get("2.1") or {"valor": None, "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # Chaves fixas por componente e ano
            chave_date_21 = f"dt_21_{ano_sel}"
            chave_link_21 = f"l_21_txt_{ano_sel}"
            ...
            with col_d21:
                ...
                # Exibição visual da pontuação recalculada em tempo real na interface
            ...
            with col_j21:
                ...
                if links_21_visuais:
                    placeholder_links_21.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_21_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 2.1
            if st.button("💾 Salvar Quesito 2.1", key=f"btn_salvar_q21_{ano_sel}", type="primary"):
                # Cálculo da pontuação conforme a regra de data
                if data_sel >= date(ano_sel, 1, 1) and data_sel.year == ano_sel:
                    pts_calc_21 = 30.0
                else:
                    pts_calc_21 = 0.0

                val_data_str = str(data_sel)

                # Resgata a lista atual do histórico direto dos dados carregados para NÃO sobrescrever
                coments_atuais_21 = res_data.get("2.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_21,
                    link=link_21,
                    comentarios=coments_atuais_21
                )

                # ATUALIZA A MEMÓRIA LOCAL: Garante atualização em tempo real sem rerun
                if "2.1" not in res_data:
                    res_data["2.1"] = {}
                res_data["2.1"]["pontos"] = pts_calc_21
                res_data["2.1"]["valor"] = val_data_str
                res_data["2.1"]["link"] = link_21

                st.toast("Quesito 2.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links (caso o modal esteja implementado)
                links_encontrados_21 = re.findall(r'https?://[^\s]+', link_21 or "")
                if links_encontrados_21 and 'modal_aviso_link' in globals():
                    modal_aviso_link("2.1", links_encontrados_21)

            # Exibição dinamicamente atualizada da pontuação
            d21_atualizado = res_data.get("2.1", {})
            pts_atuais_21 = d21_atualizado.get("pontos", 0.0)
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_3_1_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 3.1.1 - Data do Último Treinamento de Voluntários", expanded=True):
            ...
            * 🚫 **Observação:** Treinamentos em {ano_sel + 1} não pontuam.
            ...
            d311 = res_data.get("3.1.1") or {"valor": None, "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 3.1.1
            if st.button("💾 Salvar Quesito 3.1.1", key=f"btn_salvar_q311_{ano_sel}", type="primary"):
                ...
                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_311 = res_data.get("3.1.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_311,
                    link=link_311,
                    comentarios=coments_atuais_311
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "3.1.1" not in res_data:
                    res_data["3.1.1"] = {}
                res_data["3.1.1"]["pontos"] = pts_calc_311
                res_data["3.1.1"]["valor"] = val_str_311
                res_data["3.1.1"]["link"] = link_311

                st.toast("Quesito 3.1.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_311 = re.findall(r'https?://[^\s]+', link_311 or "")
                if links_encontrados_311 and 'modal_aviso_link' in globals():
                    modal_aviso_link("3.1.1", links_encontrados_311)

            # Exibição dinamicamente atualizada da pontuação
            d311_atualizado = res_data.get("3.1.1", {})
            pts_atuais_311 = d311_atualizado.get("pontos", 0.0)
    ...
    with st.container(key=f"container_bloco_compdec_4_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 4.0 - Carta Geotécnica de Suscetibilidade", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 4.0' para registrar.*")
            ...
            # Estado inicial / persistente
            d40 = res_data.get("4.0") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            v_salvo_40 = d40.get("valor", "Selecione...")

            # Chaves fixas por componente e ano
            chave_radio_40 = f"r_40_{ano_sel}"
            ...
            with col_j40:
                link_40 = st.text_area(
                    ...
                    key=chave_link_40,
                    height=100,
                    placeholder="Cole o link do documento ou comprovante aqui..."
                )
                placeholder_links_40 = st.empty()
                links_40_visuais = [u[0] for u in re.findall(REGEX_PURE_URL, link_40 or "")]
                if links_40_visuais:
                    placeholder_links_40.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_40_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 4.0
            if st.button("💾 Salvar Quesito 4.0", key=f"btn_salvar_q40_{ano_sel}", type="primary"):
                pts_calc_40 = opcoes_40.get(val_radio_40, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_40 = res_data.get("4.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_40,
                    link=link_40,
                    comentarios=coments_atuais_40
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "4.0" not in res_data:
                    res_data["4.0"] = {}
                res_data["4.0"]["pontos"] = pts_calc_40
                res_data["4.0"]["valor"] = val_radio_40
                res_data["4.0"]["link"] = link_40

                st.toast("Quesito 4.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_40 = re.findall(r'https?://[^\s]+', link_40 or "")
                if links_encontrados_40 and 'modal_aviso_link' in globals():
                    modal_aviso_link("4.0", links_encontrados_40)

            # Exibição dinamicamente atualizada da pontuação
            d40_atualizado = res_data.get("4.0", {})
            pts_atuais_40 = d40_atualizado.get("pontos", 0.0)
            val_atual_40 = d40_atualizado.get("valor", "Selecione...")

            cor_txt_40 = "#28a745" if pts_atuais_40 == 10.0 else ("#dc3545" if val_atual_40 != "Selecione..." else "#6c757d")
            st.markdown(
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_5_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 5.0 - Mapeamento Próprio de Ameaças", expanded=True):
            ...
            # Estado inicial / persistente
            d50 = res_data.get("5.0") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 5.0
            if st.button("💾 Salvar Quesito 5.0", key=f"btn_salvar_q50_{ano_sel}", type="primary"):
                pts_calc_50 = opcoes_50.get(val_radio_50, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_50 = res_data.get("5.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_50,
                    link=link_50,
                    comentarios=coments_atuais_50
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "5.0" not in res_data:
                    res_data["5.0"] = {}
                res_data["5.0"]["pontos"] = pts_calc_50
                res_data["5.0"]["valor"] = val_radio_50
                res_data["5.0"]["link"] = link_50

                st.toast("Quesito 5.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_50 = re.findall(r'https?://[^\s]+', link_50 or "")
                if links_encontrados_50 and 'modal_aviso_link' in globals():
                    modal_aviso_link("5.0", links_encontrados_50)

            # Exibição dinamicamente atualizada da pontuação
            d50_atualizado = res_data.get("5.0", {})
            pts_atuais_50 = d50_atualizado.get("pontos", 0.0)
            val_atual_50 = d50_atualizado.get("valor", "Selecione...")

            cor_txt_50 = "#28a745" if pts_atuais_50 == 200.0 else ("#dc3545" if val_atual_50 != "Selecione..." else "#6c757d")
            st.markdown(
    ...
    with st.container(key=f"container_bloco_compdec_5_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 5.1 - Principais Ameaças Identificadas", expanded=True):
            ...
            # Recupera o estado salvo no dicionário de dados
            d51 = res_data.get("5.1") or {"valor": "[]", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 5.1
            if st.button("💾 Salvar Quesito 5.1", key=f"btn_salvar_q51_{ano_sel}", type="primary"):
                ...
                pts_calc_51 = 0.0  # Quesito estritamente informativo

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_51 = res_data.get("5.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_51,
                    link=link_51,
                    comentarios=coments_atuais_51
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "5.1" not in res_data:
                    res_data["5.1"] = {}
                res_data["5.1"]["pontos"] = pts_calc_51
                res_data["5.1"]["valor"] = val_str_51
                res_data["5.1"]["link"] = link_51

                st.toast("Opções do Quesito 5.1 salvas com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_51 = re.findall(r'https?://[^\s]+', link_51 or "")
                if links_encontrados_51 and 'modal_aviso_link' in globals():
                    modal_aviso_link("5.1", links_encontrados_51)
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_5_1_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 5.1.1 - Fiscalização das Áreas de Risco", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 5.1.1' para registrar.*")
            ...
            with col_j511:
                link_511 = st.text_area(
                    "Evidência (5.1.1):",
                    value=d511.get("link", ""),
                    key=chave_link_511,
                    height=135,
                    placeholder="Cole os links de relatórios de fiscalização ou documentos comprobatórios..."
                )
    ...
    with st.container(key=f"container_bloco_compdec_5_1_2_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 5.1.2 - Possibilidade de Ocupação/Invasão em Áreas de Risco", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 5.1.2' para registrar.*")
            ...
            with col_j512:
                link_512 = st.text_area(
                    "Justificativa / Evidência (5.1.2):",
                    value=d512.get("link", ""),
                    key=chave_link_512,
                    height=100,
                    placeholder="Especifique os locais ou insira links de relatórios..."
                )
 ...
 # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_5_1_2_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 5.1.2.1 - Mecanismos para Vedar Novas Ocupações", expanded=True):
            ...
            # Estado inicial / persistente
            d5121 = res_data.get("5.1.2.1") or {"valor": "[]", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 5.1.2.1
            if st.button("💾 Salvar Quesito 5.1.2.1", key=f"btn_salvar_5_1_2_1_{ano_sel}", type="primary"):
                ...
                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_5121 = res_data.get("5.1.2.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_5121,
                    link=link_5121,
                    comentarios=coments_atuais_5121
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "5.1.2.1" not in res_data:
                    res_data["5.1.2.1"] = {}
                res_data["5.1.2.1"]["pontos"] = pts_5121
                res_data["5.1.2.1"]["valor"] = val_str_5121
                res_data["5.1.2.1"]["link"] = link_5121

                st.toast("Opções do Quesito 5.1.2.1 salvas com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_5121 = re.findall(r'https?://[^\s]+', link_5121 or "")
                if links_encontrados_5121 and 'modal_aviso_link' in globals():
                    modal_aviso_link("5.1.2.1", links_encontrados_5121)
    ...
    with st.container(key=f"container_bloco_compdec_5_2_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 5.2 - Informação à População sobre Ameaças", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 5.2' para registrar.*")
            ...
            # Estado inicial / persistente
            d52 = res_data.get("5.2") or {"valor": "Selecione...", "pontos": -50.0, "link": "", "comentarios": []}
            v_salvo_52 = d52.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_52 = f"r_52_{ano_sel}"
            chave_link_52 = f"l_52_txt_{ano_sel}"
            ...
            with col_j52:
                link_52 = st.text_area(
                    ...
                    value=d52.get("link", ""),
                    key=chave_link_52,
                    placeholder="Cole os links de matérias, boletins oficiais, campanhas ou redes sociais...",
                    height=135
                )
                placeholder_links_52 = st.empty()
                links_52_visuais = [u[0] for u in re.findall(REGEX_PURE_URL, link_52 or "")]
                if links_52_visuais:
                    placeholder_links_52.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_52_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 5.2
            if st.button("💾 Salvar Quesito 5.2", key=f"btn_salvar_5_2_{ano_sel}", type="primary"):
                pts_calc_52 = opcoes_52.get(val_radio_52, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_52 = res_data.get("5.2", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_52,
                    link=link_52,
                    comentarios=coments_atuais_52
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "5.2" not in res_data:
                    res_data["5.2"] = {}
                res_data["5.2"]["pontos"] = pts_calc_52
                res_data["5.2"]["valor"] = val_radio_52
                res_data["5.2"]["link"] = link_52

                st.toast("Quesito 5.2 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_52 = re.findall(r'https?://[^\s]+', link_52 or "")
                if links_encontrados_52 and 'modal_aviso_link' in globals():
                    modal_aviso_link("5.2", links_encontrados_52)

            # Exibição dinamicamente atualizada da pontuação
            d52_atualizado = res_data.get("5.2", {})
            pts_atuais_52 = d52_atualizado.get("pontos", 0.0)
            val_atual_52 = d52_atualizado.get("valor", "Selecione...")

            cor_txt_52 = "#28a745" if pts_atuais_52 == 0.0 and val_atual_52 != "Selecione..." else ("#dc3545" if pts_atuais_52 < 0.0 else "#6c757d")
            st.markdown(
...
# =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_6_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 6.0 - Vistorias em Edificações Vulneráveis", expanded=True):
            ...
            # Estado inicial / persistente
            d60 = res_data.get("6.0") or {"valor": "Selecione...", "pontos": -50.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 6.0
            if st.button("💾 Salvar Quesito 6.0", key=f"btn_salvar_6_0_{ano_sel}", type="primary"):
                pts_calc_60 = opcoes_60.get(val_radio_60, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_60 = res_data.get("6.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_60,
                    link=link_60,
                    comentarios=coments_atuais_60
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "6.0" not in res_data:
                    res_data["6.0"] = {}
                res_data["6.0"]["pontos"] = pts_calc_60
                res_data["6.0"]["valor"] = val_radio_60
                res_data["6.0"]["link"] = link_60

                st.toast("Quesito 6.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_60 = re.findall(r'https?://[^\s]+', link_60 or "")
                if links_encontrados_60 and 'modal_aviso_link' in globals():
                    modal_aviso_link("6.0", links_encontrados_60)

            # Exibição dinamicamente atualizada da pontuação
            d60_atualizado = res_data.get("6.0", {})
            pts_atuais_60 = d60_atualizado.get("pontos", 0.0)
            val_atual_60 = d60_atualizado.get("valor", "Selecione...")

            cor_txt_60 = "#28a745" if pts_atuais_60 == 0.0 and val_atual_60 != "Selecione..." else ("#dc3545" if pts_atuais_60 < 0.0 else "#6c757d")
            st.markdown(
    ...
    with st.container(key=f"container_bloco_compdec_7_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.0 - Plano de Contingência Municipal (PLANCON)", expanded=True):
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 7.0
            if st.button("💾 Salvar Quesito 7.0", key=f"btn_salvar_7_0_{ano_sel}", type="primary"):
                pts_calc_70 = opcoes_70.get(val_radio_70, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_70 = res_data.get("7.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_70,
                    link=link_70,
                    comentarios=coments_atuais_70
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "7.0" not in res_data:
                    res_data["7.0"] = {}
                res_data["7.0"]["pontos"] = pts_calc_70
                res_data["7.0"]["valor"] = val_radio_70
                res_data["7.0"]["link"] = link_70

                st.toast("Quesito 7.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_70 = re.findall(r'https?://[^\s]+', link_70 or "")
                if links_encontrados_70 and 'modal_aviso_link' in globals():
                    modal_aviso_link("7.0", links_encontrados_70)

            # Exibição dinamicamente atualizada da pontuação
            d70_atualizado = res_data.get("7.0", {})
            pts_atuais_70 = d70_atualizado.get("pontos", 0.0)
            val_atual_70 = d70_atualizado.get("valor", "Selecione...")

            cor_txt_70 = "#28a745" if pts_atuais_70 == 50.0 else ("#dc3545" if val_atual_70 != "Selecione..." and pts_atuais_70 == 0.0 else "#6c757d")
            st.markdown(
        ...
        with st.expander("📌 Quesito 7.2 - Exercícios Simulados para Contingências", expanded=True):
            ...
            with col_j72:
                link_72 = st.text_area(
                    ...
                    value=d72.get("link", ""),
                    key=chave_link_72,
                    placeholder="Cole os links de relatórios ou evidências dos simulados...",
                    height=100
                )
    ...
    with st.container(key=f"container_bloco_compdec_7_3_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.3 - Sistema de Alerta para Desastres", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 7.3' para registrar.*")
            ...
            # Estado inicial / persistente
            d73 = res_data.get("7.3") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            v_salvo_73 = d73.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_73 = f"r_73_{ano_sel}"
            chave_link_73 = f"l_73_txt_{ano_sel}"
            ...
            with col_j73:
                link_73 = st.text_area(
                    ...
                    value=d73.get("link", ""),
                    key=chave_link_73,
                    placeholder="Descreva o sistema de alerta ou insira os links de comprovação...",
                    height=100
                )
                placeholder_links_73 = st.empty()
                links_73_visuais = [u[0] for u in re.findall(REGEX_PURE_URL, link_73 or "")]
                if links_73_visuais:
                    placeholder_links_73.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_73_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 7.3
            if st.button("💾 Salvar Quesito 7.3", key=f"btn_salvar_7_3_{ano_sel}", type="primary"):
                pts_calc_73 = opcoes_73.get(val_radio_73, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_73 = res_data.get("7.3", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_73,
                    link=link_73,
                    comentarios=coments_atuais_73
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "7.3" not in res_data:
                    res_data["7.3"] = {}
                res_data["7.3"]["pontos"] = pts_calc_73
                res_data["7.3"]["valor"] = val_radio_73
                res_data["7.3"]["link"] = link_73

                st.toast("Quesito 7.3 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_73 = re.findall(r'https?://[^\s]+', link_73 or "")
                if links_encontrados_73 and 'modal_aviso_link' in globals():
                    modal_aviso_link("7.3", links_encontrados_73)

            # Exibição dinamicamente atualizada da pontuação
            d73_atualizado = res_data.get("7.3", {})
            pts_atuais_73 = d73_atualizado.get("pontos", 0.0)
            val_atual_73 = d73_atualizado.get("valor", "Selecione...")

            cor_txt_73 = "#28a745" if pts_atuais_73 == 50.0 else ("#dc3545" if val_atual_73 != "Selecione..." and pts_atuais_73 == 0.0 else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_73}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 7.3: {pts_atuais_73:.1f} pontos</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_7_3_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.3.1 - Tipos de Sistemas de Alerta Utilizados", expanded=True):
            st.subheader("7.3.1 • Tipos de Alerta")
            st.write("**Assinale os tipos de sistemas de alerta utilizados pelo Município:**")
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 7.3.1' para registrar.*")

            # Estado inicial / persistente
            d731 = res_data.get("7.3.1") or {"valor": "[]", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # Chaves fixas para os componentes do Streamlit
            chave_link_731 = f"l_731_txt_{ano_sel}"
            ...
            with col_j731:
                link_731 = st.text_area(
                    ...
                    value=d731.get("link", ""),
                    key=chave_link_731,
                    placeholder="Detalhamento sobre os tipos de alerta ou insira os links...",
                    height=200
                )
                placeholder_links_731 = st.empty()
                links_731_visuais = [u[0] for u in re.findall(REGEX_PURE_URL, link_731 or "")]
                if links_731_visuais:
                    placeholder_links_731.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_731_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 7.3.1
            if st.button("💾 Salvar Quesito 7.3.1", key=f"btn_salvar_7_3_1_{ano_sel}", type="primary"):
                ...
                val_str_731 = str(sel_731)
                pts_731 = 0.0  # Quesito informativo

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_731 = res_data.get("7.3.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_731,
                    link=link_731,
                    comentarios=coments_atuais_731
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "7.3.1" not in res_data:
                    res_data["7.3.1"] = {}
                res_data["7.3.1"]["pontos"] = pts_731
                res_data["7.3.1"]["valor"] = val_str_731
                res_data["7.3.1"]["link"] = link_731

                st.toast("Quesito 7.3.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_731 = re.findall(r'https?://[^\s]+', link_731 or "")
                if links_encontrados_731 and 'modal_aviso_link' in globals():
                    modal_aviso_link("7.3.1", links_encontrados_731)

            # Exibição do status visual de impacto da pontuação
    ...
    with st.container(key=f"container_bloco_compdec_7_4_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 7.4 - Dispositivo ou Sistema de Alarme", expanded=True):
            ...
            # Estado inicial / persistente
            d74 = res_data.get("7.4") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 7.4
            if st.button("💾 Salvar Quesito 7.4", key=f"btn_salvar_7_4_{ano_sel}", type="primary"):
                pts_calc_74 = opcoes_74.get(val_radio_74, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_74 = res_data.get("7.4", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_74,
                    link=link_74,
                    comentarios=coments_atuais_74
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "7.4" not in res_data:
                    res_data["7.4"] = {}
                res_data["7.4"]["pontos"] = pts_calc_74
                res_data["7.4"]["valor"] = val_radio_74
                res_data["7.4"]["link"] = link_74

                st.toast("Quesito 7.4 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_74 = re.findall(r'https?://[^\s]+', link_74 or "")
                if links_encontrados_74 and 'modal_aviso_link' in globals():
                    modal_aviso_link("7.4", links_encontrados_74)

            # Exibição dinamicamente atualizada da pontuação
            d74_atualizado = res_data.get("7.4", {})
            pts_atuais_74 = d74_atualizado.get("pontos", 0.0)
            val_atual_74 = d74_atualizado.get("valor", "Selecione...")

            cor_txt_74 = "#28a745" if pts_atuais_74 == 50.0 else ("#dc3545" if val_atual_74 != "Selecione..." and pts_atuais_74 == 0.0 else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_74}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 7.4: {pts_atuais_74:.1f} pontos</span>",
                unsafe_allow_html=True
            )
...
# =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_8_2_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 8.2 - Registro Eletrônico de Ocorrências", expanded=True):
            ...
            # Recupera o estado salvo no dicionário de dados
            d82 = res_data.get("8.2") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentario": ""}
            v_salvo_82 = d82.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_82 = f"r_82_{ano_sel}"
            chave_link_82 = f"l_82_txt_{ano_sel}"
            ...
            with col_j82:
                ...
                if links_82_visuais:
                    placeholder_links_82.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_82_visuais]))
            ...
            if st.button("💾 Salvar Quesito 8.2", key=f"btn_salvar_8_2_{ano_sel}", type="primary"):
                ...
                if link_82 != d82.get("link", "") and links_atuais and links_atuais != links_antigos:
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_9_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 9.0 - Estudo de Estrutura de Escolas e Unidades de Saúde", expanded=True):
            ...
            if st.button("💾 Salvar Quesito 9.0", key=f"btn_salvar_9_0_{ano_sel}", type="primary"):
                pts_calc_90 = opcoes_90.get(val_radio_90, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_90 = res_data.get("9.0", {}).get("comentarios", [])
                ...
                res_data["9.0"]["valor"] = val_radio_90
                res_data["9.0"]["link"] = link_90

                st.toast("Quesito 9.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_90 = re.findall(r'https?://[^\s]+', link_90 or "")
                if links_encontrados_90 and 'modal_aviso_link' in globals():
                    modal_aviso_link("9.0", links_encontrados_90)

            # Exibição dinamicamente atualizada da pontuação
            d90_atualizado = res_data.get("9.0", {})
            pts_atuais_90 = d90_atualizado.get("pontos", 0.0)
            val_atual_90 = d90_atualizado.get("valor", "Selecione...")
    ...
    with st.container(key=f"container_bloco_compdec_10_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 10.0 - Plano de Mobilidade Urbana", expanded=True):
            ...
            # Chaves fixas para os componentes do Streamlit
            chave_radio_100 = f"r_100_{ano_sel}"
            chave_link_100 = f"l_100_txt_{ano_sel}"
            ...
            with col_j100:
                ...
                if links_100_visuais:
                    placeholder_links_100.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_100_visuais]))
            ...
            if st.button("💾 Salvar Quesito 10.0", key=f"btn_salvar_10_0_{ano_sel}", type="primary"):
                pts_calc_100 = opcoes_100.get(val_radio_100, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_100 = res_data.get("10.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_100,
                    link=link_100,
                    comentarios=coments_atuais_100
                )
                ...
                res_data["10.0"]["valor"] = val_radio_100
                res_data["10.0"]["link"] = link_100

                st.toast("Quesito 10.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_100 = re.findall(r'https?://[^\s]+', link_100 or "")
                if links_encontrados_100 and 'modal_aviso_link' in globals():
                    modal_aviso_link("10.0", links_encontrados_100)

            # Exibição dinamicamente atualizada da pontuação
            d100_atualizado = res_data.get("10.0", {})
            pts_atuais_100 = d100_atualizado.get("pontos", 0.0)
            val_atual_100 = d100_atualizado.get("valor", "Selecione...")
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_11_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.0 - Existência de Transporte Público Coletivo", expanded=True):
            st.subheader("11.0 • Transporte Coletivo")
            st.write("**No Município existe transporte público coletivo?**")
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 11.0' para registrar.*")
            ...
            # Chaves fixas para os componentes do Streamlit
            chave_radio_110 = f"r_110_{ano_sel}"
            ...
            with col_j110:
                ...
                if links_110_visuais:
                    placeholder_links_110.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_110_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.0
            if st.button("💾 Salvar Quesito 11.0", key=f"btn_salvar_11_0_{ano_sel}", type="primary"):
                pts_110 = 0.0  # Quesito de diagnóstico inicial (0.0 pontos)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_110 = res_data.get("11.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_110,
                    link=link_110,
                    comentarios=coments_atuais_110
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.0" not in res_data:
                    res_data["11.0"] = {}
                res_data["11.0"]["pontos"] = pts_110
                res_data["11.0"]["valor"] = val_radio_110
                res_data["11.0"]["link"] = link_110

                st.toast("Quesito 11.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_110 = re.findall(r'https?://[^\s]+', link_110 or "")
                if links_encontrados_110 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.0", links_encontrados_110)
    ...
    with st.container(key=f"container_bloco_compdec_11_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.1 - Metas de Qualidade e Desempenho", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 11.1' para registrar.*")
            ...
            # Estado inicial / persistente
            d111 = res_data.get("11.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            v_salvo_111 = d111.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_111 = f"r_111_{ano_sel}"
            ...
            with col_j111:
                ...
                if links_111_visuais:
                    placeholder_links_111.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_111_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.1
            if st.button("💾 Salvar Quesito 11.1", key=f"btn_salvar_11_1_{ano_sel}", type="primary"):
                pts_calc_111 = opts111.get(val_radio_111, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_111 = res_data.get("11.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_111,
                    link=link_111,
                    comentarios=coments_atuais_111
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.1" not in res_data:
                    res_data["11.1"] = {}
                res_data["11.1"]["pontos"] = pts_calc_111
                res_data["11.1"]["valor"] = val_radio_111
                res_data["11.1"]["link"] = link_111

                st.toast("Quesito 11.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_111 = re.findall(r'https?://[^\s]+', link_111 or "")
                if links_encontrados_111 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.1", links_encontrados_111)

            # Exibição dinamicamente atualizada da pontuação
            d111_atualizado = res_data.get("11.1", {})
            pts_atuais_111 = d111_atualizado.get("pontos", 0.0)
            val_atual_111 = d111_atualizado.get("valor", "Selecione...")

            cor_txt_111 = "#dc3545" if pts_atuais_111 < 0.0 else ("#28a745" if val_atual_111 == "Sim (00 pts)" else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_111}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 11.1: {pts_atuais_111:.1f} pontos</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_11_1_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.1.1 - Atingimento de Metas de Desempenho", expanded=True):
            st.subheader("11.1.1 • Cumprimento do Plano de Metas")
            st.write("**As metas de qualidade e desempenho estão sendo atingidas?**")
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 11.1.1' para registrar.*")
            ...
            # Estado inicial / persistente
            d1111 = res_data.get("11.1.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            v_salvo_1111 = d1111.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_1111 = f"r_1111_{ano_sel}"
            chave_link_1111 = f"l_1111_txt_{ano_sel}"
            ...
            with col_j1111:
                ...
                if links_1111_visuais:
                    placeholder_links_1111.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_1111_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.1.1
            if st.button("💾 Salvar Quesito 11.1.1", key=f"btn_salvar_11_1_1_{ano_sel}", type="primary"):
                pts_calc_1111 = opts1111.get(val_radio_1111, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_1111 = res_data.get("11.1.1", {}).get("comentarios", [])

                save_resp(
                    qid="11.1.1",
                    valor=val_radio_1111,
                    pontos=pts_calc_1111,
                    link=link_1111,
                    comentarios=coments_atuais_1111
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.1.1" not in res_data:
                    res_data["11.1.1"] = {}
                res_data["11.1.1"]["pontos"] = pts_calc_1111
                res_data["11.1.1"]["valor"] = val_radio_1111
                res_data["11.1.1"]["link"] = link_1111

                st.toast("Quesito 11.1.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_1111 = re.findall(r'https?://[^\s]+', link_1111 or "")
                if links_encontrados_1111 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.1.1", links_encontrados_1111)

            # Exibição dinamicamente atualizada da pontuação
            d1111_atualizado = res_data.get("11.1.1", {})
            pts_atuais_1111 = d1111_atualizado.get("pontos", 0.0)
            val_atual_1111 = d1111_atualizado.get("valor", "Selecione...")

            cor_txt_1111 = "#dc3545" if pts_atuais_1111 < 0.0 else ("#28a745" if val_atual_1111 == "Todas as metas foram atingidas (00 pts)" else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_1111}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 11.1.1: {pts_atuais_1111:.1f} pontos</span>",
                unsafe_allow_html=True
            )
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_11_1_1_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.1.1.1 - Aplicação de Sanções Administrativas", expanded=True):
            st.subheader("11.1.1.1 • Penalidades")
            st.write("**Foi aplicada penalidade pela meta não cumprida?**")
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 11.1.1.1' para registrar.*")
            ...
            # Estado inicial / persistente
            d11111 = res_data.get("11.1.1.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            v_salvo_11111 = d11111.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_11111 = f"r_11111_{ano_sel}"
            chave_link_11111 = f"l_11111_txt_{ano_sel}"
            ...
            with col_j11111:
                link_11111 = st.text_area(
                    "Auto de Infração (11.1.1.1):",
                    value=d11111.get("link", ""),
                    key=chave_link_11111,
                    placeholder="Ex: Link de publicação do diário oficial de multas aplicadas, rescisões...",
                    height=110
                )
                placeholder_links_11111 = st.empty()
                links_11111_visuais = [u[0] for u in re.findall(REGEX_PURE_URL, link_11111 or "")]
                if links_11111_visuais:
                    placeholder_links_11111.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_11111_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.1.1.1
            if st.button("💾 Salvar Quesito 11.1.1.1", key=f"btn_salvar_11_1_1_1_{ano_sel}", type="primary"):
                pts_calc_11111 = opcoes_11111.get(val_radio_11111, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_11111 = res_data.get("11.1.1.1", {}).get("comentarios", [])

                save_resp(
                    qid="11.1.1.1",
                    valor=val_radio_11111,
                    pontos=pts_calc_11111,
                    link=link_11111,
                    comentarios=coments_atuais_11111
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.1.1.1" not in res_data:
                    res_data["11.1.1.1"] = {}
                res_data["11.1.1.1"]["pontos"] = pts_calc_11111
                res_data["11.1.1.1"]["valor"] = val_radio_11111
                res_data["11.1.1.1"]["link"] = link_11111

                st.toast("Quesito 11.1.1.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_11111 = re.findall(r'https?://[^\s]+', link_11111 or "")
                if links_encontrados_11111 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.1.1.1", links_encontrados_11111)

            # Exibição dinamicamente atualizada da pontuação
            d11111_atualizado = res_data.get("11.1.1.1", {})
            pts_atuais_11111 = d11111_atualizado.get("pontos", 0.0)
            val_atual_11111 = d11111_atualizado.get("valor", "Selecione...")

            cor_txt_11111 = "#dc3545" if pts_atuais_11111 < 0.0 else ("#28a745" if val_atual_11111 == "Sim (00 pts)" else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_11111}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 11.1.1.1: {pts_atuais_11111:.1f} pontos</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_11_2_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.2 - Pesquisa de Satisfação dos Usuários", expanded=True):
            st.subheader("11.2 • Pesquisa de Satisfação")
            
            # Pega apenas os números do ano
            ...
            # Estado inicial / persistente
            d112 = res_data.get("11.2") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.2
            if st.button("💾 Salvar Quesito 11.2", key=f"btn_salvar_11_2_{ano_sel}", type="primary"):
                pts_calc_112 = opcoes_112.get(val_radio_112, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_112 = res_data.get("11.2", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_112,
                    link=link_112,
                    comentarios=coments_atuais_112
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.2" not in res_data:
                    res_data["11.2"] = {}
                res_data["11.2"]["pontos"] = pts_calc_112
                res_data["11.2"]["valor"] = val_radio_112
                res_data["11.2"]["link"] = link_112

                st.toast("Quesito 11.2 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_112 = re.findall(r'https?://[^\s]+', link_112 or "")
                if links_encontrados_112 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.2", links_encontrados_112)

            # Exibição dinamicamente atualizada da pontuação
            d112_atualizado = res_data.get("11.2", {})
            pts_atuais_112 = d112_atualizado.get("pontos", 0.0)
            val_atual_112 = d112_atualizado.get("valor", "Selecione...")

            cor_txt_112 = "#dc3545" if pts_atuais_112 < 0.0 else ("#28a745" if val_atual_112 == "Sim (00 pts)" else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_112}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 11.2: {pts_atuais_112:.1f} pontos</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_11_2_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.2.1 - Ações com Base na Pesquisa de Satisfação", expanded=True):
            st.subheader("11.2.1 • Ações Pós-Pesquisa")
            st.write("**Foram realizadas ações com base nesta pesquisa?**")
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 11.2.1' para registrar.*")
            ...
            # Estado inicial / persistente
            d1121 = res_data.get("11.2.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            v_salvo_1121 = d1121.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_1121 = f"r_1121_{ano_sel}"
            chave_link_1121 = f"l_1121_txt_{ano_sel}"
            ...
            with col_j1121:
                ...
                if links_1121_visuais:
                    placeholder_links_1121.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_1121_visuais]))
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.2.1
            if st.button("💾 Salvar Quesito 11.2.1", key=f"btn_salvar_11_2_1_{ano_sel}", type="primary"):
                pts_calc_1121 = opcoes_1121.get(val_radio_1121, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_1121 = res_data.get("11.2.1", {}).get("comentarios", [])

                save_resp(
                    qid="11.2.1",
                    valor=val_radio_1121,
                    pontos=pts_calc_1121,
                    link=link_1121,
                    comentarios=coments_atuais_1121
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.2.1" not in res_data:
                    res_data["11.2.1"] = {}
                res_data["11.2.1"]["pontos"] = pts_calc_1121
                res_data["11.2.1"]["valor"] = val_radio_1121
                res_data["11.2.1"]["link"] = link_1121

                st.toast("Quesito 11.2.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_1121 = re.findall(r'https?://[^\s]+', link_1121 or "")
                if links_encontrados_1121 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.2.1", links_encontrados_1121)

            # Exibição dinamicamente atualizada da pontuação
            d1121_atualizado = res_data.get("11.2.1", {})
            pts_atuais_1121 = d1121_atualizado.get("pontos", 0.0)
            val_atual_1121 = d1121_atualizado.get("valor", "Selecione...")

            cor_txt_1121 = "#dc3545" if pts_atuais_1121 < 0.0 else ("#28a745" if val_atual_1121 == "Sim (00 pts)" else "#6c757d")
            st.markdown(
                f"<span style='color:{cor_txt_1121}; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 11.2.1: {pts_atuais_1121:.1f} pontos</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_11_3_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.3 - Resultado Financeiro do Transporte Público", expanded=True):
            ...
            # Estado inicial / persistente
            d113 = res_data.get("11.3") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.3
            if st.button("💾 Salvar Quesito 11.3", key=f"btn_salvar_11_3_{ano_sel}", type="primary"):
                pts_calc_113 = opcoes_113.get(val_radio_113, 0.0)

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_113 = res_data.get("11.3", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=pts_calc_113,
                    link=link_113,
                    comentarios=coments_atuais_113
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.3" not in res_data:
                    res_data["11.3"] = {}
                res_data["11.3"]["pontos"] = pts_calc_113
                res_data["11.3"]["valor"] = val_radio_113
                res_data["11.3"]["link"] = link_113

                st.toast("Quesito 11.3 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_113 = re.findall(r'https?://[^\s]+', link_113 or "")
                if links_encontrados_113 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.3", links_encontrados_113)

            # Exibição dinamicamente atualizada da pontuação
            d113_atualizado = res_data.get("11.3", {})
            pts_atuais_113 = d113_atualizado.get("pontos", 0.0)
            
            st.markdown(
                f"<span style='color:#28a745; font-weight:bold;'>"
                f"📊 Impacto de Pontuação no Quesito 11.3: {pts_atuais_113:.1f} pontos (Diagnóstico Financeiro)</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_11_3_1_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 11.3.1 - Transparência dos Benefícios Tarifários", expanded=True):
            ...
            # Estado inicial / persistente
            d1131 = res_data.get("11.3.1") or {"valor": "Link fornecido", "pontos": 0.0, "link": "", "justificativa": "", "comentarios": []}
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 11.3.1
            if st.button("💾 Salvar Quesito 11.3.1", key=f"btn_salvar_11_3_1_{ano_sel}", type="primary"):
                lnk_inserido = (link_1131 or "").strip()
                val_1131 = "Não disponível (XYZ)" if lnk_inserido.upper() == "XYZ" else "Link fornecido"

                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_1131 = res_data.get("11.3.1", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=0.0,
                    link=lnk_inserido,
                    comentarios=coments_atuais_1131,
                    justificativa=justificativa_1131
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "11.3.1" not in res_data:
                    res_data["11.3.1"] = {}
                res_data["11.3.1"]["pontos"] = 0.0
                res_data["11.3.1"]["valor"] = val_1131
                res_data["11.3.1"]["link"] = lnk_inserido
                res_data["11.3.1"]["justificativa"] = justificativa_1131

                st.toast("Quesito 11.3.1 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_1131 = re.findall(r'https?://[^\s]+', lnk_inserido or "")
                if links_encontrados_1131 and 'modal_aviso_link' in globals():
                    modal_aviso_link("11.3.1", links_encontrados_1131)

            # Exibição dinamicamente atualizada da pontuação
            st.markdown(
                "<span style='color:#28a745; font-weight:bold;'>"
                "📊 Impacto de Pontuação no Quesito 11.3.1: 0.0 pontos (Informativo / Transparência)</span>",
                unsafe_allow_html=True
            )
    ...
    with st.container(key=f"container_bloco_compdec_12_0_final_{ano_sel}", border=True):
        with st.expander("📌 Quesito 12.0 - Transporte Remunerado Privado Individual (App)", expanded=True):
            ...
            with col_r120:
                lista_opcoes_120 = opcoes_120
                idx_120 = lista_opcoes_120.index(v_salvo_120) if v_salvo_120 in lista_opcoes_120 else 0

                val_radio_120 = st.radio(
            ...
            # BOTÃO DE SALVAMENTO PRINCIPAL DO QUESITO 12.0
            if st.button("💾 Salvar Quesito 12.0", key=f"btn_salvar_12_0_{ano_sel}", type="primary"):
                # Resgata a lista atual do histórico direto dos dados carregados
                coments_atuais_120 = res_data.get("12.0", {}).get("comentarios", [])
                ...
                save_resp(
                    ...
                    pontos=0.0,
                    link=link_120,
                    comentarios=coments_atuais_120
                )

                # ATUALIZA A MEMÓRIA LOCAL: Atualização reativa em tempo real
                if "12.0" not in res_data:
                    res_data["12.0"] = {}
                res_data["12.0"]["pontos"] = 0.0
                res_data["12.0"]["valor"] = val_radio_120
                res_data["12.0"]["link"] = link_120

                st.toast("Quesito 12.0 salvo com sucesso!", icon="✅")

                # Validação/Aviso complementar de links
                links_encontrados_120 = re.findall(r'https?://[^\s]+', link_120 or "")
                if links_encontrados_120 and 'modal_aviso_link' in globals():
                    modal_aviso_link("12.0", links_encontrados_120)

            # Exibição dinamicamente atualizada da pontuação
            st.markdown(
                "<span style='color:#28a745; font-weight:bold;'>"
                "📊 Impacto de Pontuação no Quesito 12.0: 0.0 pontos (Informativo)</span>",
                unsafe_allow_html=True
            )
    # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_12_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito 12.1 - Regulamentação do Transporte por Aplicativo", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 12.1' para registrar.*")
            ...
            with col_j121:
                ...
                if links_121_visuais:
                    placeholder_links_121.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_121_visuais]))
            ...
            if st.button("💾 Salvar Quesito 12.1", key=f"btn_salvar_12_1_{ano_sel}", type="primary"):
                ...
                val_selecionado_121 = st.session_state.get(chave_radio_121, v_salvo_121)
                pts_121 = opts121.get(val_selecionado_121, 0.0)
                comentario_para_salvar = st.session_state.get(chave_coment_121, d121.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("12.1", val_selecionado_121, pts_121, link_121, comentario_para_salvar)

                # 3. Atualiza a estrutura no dicionário local res_data
                res_data["12.1"] = {
                    "valor": val_selecionado_121,
                    "pontos": pts_121,
                    "link": link_121,
   ...
   # =============================================================================
    ...
    with st.container(key=f"container_bloco_compdec_12_1_3_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito 12.1.3.1 - Periodicidade e Evidência das Ações", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 12.1.3.1' para registrar.*")

            perio_opcoes = ["Selecione...", "Diariamente", "Semanalmente", "Mensalmente", "Anualmente"]

            # Recupera o estado salvo no dicionário de dados
            d12131 = res_data.get("12.1.3.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentario": ""}
            v_salvo_12131 = d12131.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_12131 = f"r_12131_{ano_sel}"
            chave_link_12131 = f"l_12131_txt_{ano_sel}"
            chave_coment_12131 = f"coment_12.1.3.1_{ano_sel}"
            ...
            with col_j12131:
                ...
                if links_12131_visuais:
                    placeholder_links_12131.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_12131_visuais]))
            ...
            # BOTÃO DE SALVAMENTO MANUAL
            # -----------------------------------------------------------------
            if st.button("💾 Salvar Quesito 12.1.3.1", key=f"btn_salvar_12_1_3_1_{ano_sel}", type="primary"):
                # 1. Coleta os dados dos campos do Streamlit
                val_selecionado_12131 = st.session_state.get(chave_radio_12131, v_salvo_12131)
                pts_12131 = 0.0  # Quesito informativo de rotina
                comentario_para_salvar = st.session_state.get(chave_coment_12131, d12131.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("12.1.3.1", val_selecionado_12131, pts_12131, link_12131, comentario_para_salvar)

                # 3. Atualiza a estrutura no dicionário local res_data
                res_data["12.1.3.1"] = {
                    "valor": val_selecionado_12131,
                    "pontos": pts_12131,
                    "link": link_12131,
                    "comentario": comentario_para_salvar
                }
    ...
    # GATILHO DO MODAL 12.1.3.1 (Fora do container principal)
    ...
    with st.container(key=f"container_bloco_compdec_13_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito 13.1 - Detalhamento das Ações Realizadas em {ano_anterior}", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 13.1' para registrar.*")
            ...
            # Sanitização do valor recuperado para conversão segura em lista
            ...
            # Chaves fixas para os componentes do Streamlit
            chave_link_131 = f"l_131_txt_{ano_sel}"
            ...
            if st.button("💾 Salvar Quesito 13.1", key=f"btn_salvar_13_1_{ano_sel}", type="primary"):
                # 1. Coleta os valores selecionados nos checkboxes
                sel_atual_131 = []
                for ac in acoes_131:
                    ac_key = ac.replace(" ", "_").replace("/", "_").replace("-", "_").lower()
                    if st.session_state.get(f"chk_131_{ac_key}_{ano_sel}", False):
                        sel_atual_131.append(ac)

                val_str_131 = str(sel_atual_131)
                pts_131 = 0.0  # Quesito diagnóstico cumulativo
                comentario_para_salvar = st.session_state.get(chave_coment_131, d131.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("13.1", val_str_131, pts_131, link_131, comentario_para_salvar)
    ...
    with st.container(key=f"container_bloco_compdec_13_1_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito 13.1.1 - Cronograma de Manutenção da Infraestrutura", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 13.1.1' para registrar.*")

            opcoes_1311 = ["Selecione...", "Sim (00 pts)", "Não (-20 pts)"]

            # Recupera o estado salvo no dicionário de dados
            d1311 = res_data.get("13.1.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentario": ""}
            v_salvo_1311 = d1311.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_1311 = f"r_1311_{ano_sel}"
            chave_link_1311 = f"l_1311_txt_{ano_sel}"
            ...
            with col_j1311:
                ...
                if links_1311_visuais:
                    placeholder_links_1311.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_1311_visuais]))
            ...
            # BOTÃO DE SALVAMENTO MANUAL
            # -----------------------------------------------------------------
            if st.button("💾 Salvar Quesito 13.1.1", key=f"btn_salvar_13_1_1_{ano_sel}", type="primary"):
                # 1. Coleta os valores selecionados nos campos
                val_selecionado_1311 = st.session_state.get(chave_radio_1311, v_salvo_1311)
                pts_1311 = -20.0 if "Não" in val_selecionado_1311 else 0.0
                comentario_para_salvar = st.session_state.get(chave_coment_1311, d1311.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("13.1.1", val_selecionado_1311, pts_1311, link_1311, comentario_para_salvar)

                # 3. Atualiza a estrutura no dicionário local res_data
                res_data["13.1.1"] = {
                    "valor": val_selecionado_1311,
                    "pontos": pts_1311,
                    "link": link_1311,
                    "comentario": comentario_para_salvar
                }
    ...
    # GATILHO DO MODAL 13.1.1 (Fora do container principal)
    ...
    with st.container(key=f"container_bloco_compdec_13_1_1_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito 13.1.1.1 - Cumprimento e Execução das Manutenções Preventivas", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 13.1.1.1' para registrar.*")
            ...
            # Recupera o estado salvo no dicionário de dados
            d13111 = res_data.get("13.1.1.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentario": ""}
            v_salvo_13111 = d13111.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_13111 = f"r_13111_{ano_sel}"
            chave_link_13111 = f"l_13111_txt_{ano_sel}"
            ...
            with col_j13111:
                ...
                if links_13111_visuais:
                    placeholder_links_13111.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_13111_visuais]))
            ...
            # BOTÃO DE SALVAMENTO MANUAL
            # -----------------------------------------------------------------
            if st.button("💾 Salvar Quesito 13.1.1.1", key=f"btn_salvar_13_1_1_1_{ano_sel}", type="primary"):
                # 1. Coleta os valores selecionados nos campos
                val_selecionado_13111 = st.session_state.get(chave_radio_13111, v_salvo_13111)
                pts_13111 = float(opts13111.get(val_selecionado_13111, 0.0))
                comentario_para_salvar = st.session_state.get(chave_coment_13111, d13111.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("13.1.1.1", val_selecionado_13111, pts_13111, link_13111, comentario_para_salvar)

                # 3. Atualiza a estrutura no dicionário local res_data
                res_data["13.1.1.1"] = {
                    "valor": val_selecionado_13111,
                    "pontos": pts_13111,
                    "link": link_13111,
                    "comentario": comentario_para_salvar
                }
    ...
    # GATILHO DO MODAL 13.1.1.1 (Fora do container principal)
    ...
    with st.container(key=f"container_bloco_compdec_14_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito 14.1 - Detalhamento dos Recursos de Acessibilidade", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito 14.1' para registrar.*")

            # Recupera o estado salvo no dicionário de dados
            d141 = res_data.get("14.1") or {"valor": "[]", "pontos": 0.0, "link": "", "comentario": ""}

            # Sanitização e conversão segura da string salva para lista Python
            raw_v141 = d141.get("valor", "[]")
            if not isinstance(raw_v141, str) or not raw_v141.startswith("["):
                raw_v141 = "[]"
            try:
                lista_salva_141 = ast.literal_eval(raw_v141)
                if not isinstance(lista_salva_141, list):
                    lista_salva_141 = []
            except Exception:
                lista_salva_141 = []
            ...
            # Chaves fixas para os componentes do Streamlit
            chave_link_141 = f"l_141_txt_{ano_sel}"
            ...
            if st.button("💾 Salvar Quesito 14.1", key=f"btn_salvar_14_1_{ano_sel}", type="primary"):
                # 1. Coleta quais checkboxes de recursos foram marcados
                sel_atual_141 = []
                for rec in recursos_141:
                    rec_key = rec.replace(" ", "_").replace("ç", "c").replace("ã", "a").replace("í", "i").lower()[:20]
                    if st.session_state.get(f"chk_141_{rec_key}_{ano_sel}", False):
                        sel_atual_141.append(rec)

                val_str_141 = str(sel_atual_141)
                pts_141 = 0.0
                comentario_para_salvar = st.session_state.get(chave_coment_141, d141.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("14.1", val_str_141, pts_141, link_141, comentario_para_salvar)
  ...
  # =============================================================================
    ...
    with st.container(key=f"container_bloco_externo_c1_1_final_{ano_sel}", border=True):
        with st.expander(f"📌 Quesito C1.1 - Estágio de Classificação no Programa MCR2030", expanded=True):
            ...
            st.caption("ℹ *Preencha os campos abaixo e clique no botão 'Salvar Quesito C1.1' para registrar.*")
            ...
            # Recupera o estado salvo no dicionário de dados
            dc11 = res_data.get("C1.1") or {"valor": "Selecione...", "pontos": 0.0, "link": "", "comentario": ""}
            v_salvo_c11 = dc11.get("valor", "Selecione...")

            # Chaves fixas para os componentes do Streamlit
            chave_radio_c11 = f"r_c11_{ano_sel}"
            chave_link_c11 = f"l_c11_txt_{ano_sel}"
            chave_coment_c11 = f"coment_C1.1_{ano_sel}"
            ...
            with col_jc11:
                ...
                if links_c11_visuais:
                    placeholder_links_c11.markdown("**Links Ativos:** " + " | ".join([f"🔗 [{u}]({u})" for u in links_c11_visuais]))
            ...
            # BOTÃO DE SALVAMENTO MANUAL
            # -----------------------------------------------------------------
            if st.button("💾 Salvar Quesito C1.1", key=f"btn_salvar_c1_1_{ano_sel}", type="primary"):
                # 1. Coleta os valores selecionados nos campos
                val_selecionado_c11 = st.session_state.get(chave_radio_c11, v_salvo_c11)
                pts_c11 = float(opts_c11.get(val_selecionado_c11, 0.0))
                comentario_para_salvar = st.session_state.get(chave_coment_c11, dc11.get("comentario", ""))

                # 2. Persiste no backend / banco de dados
                save_resp("C1.1", val_selecionado_c11, pts_c11, link_c11, comentario_para_salvar)

                # 3. Atualiza a estrutura no dicionário local res_data
                res_data["C1.1"] = {
                    "valor": val_selecionado_c11,
                    "pontos": pts_c11,
                    "link": link_c11,
                    "comentario": comentario_para_salvar
                }
    ...
    # GATILHO DO MODAL C1.1 (Fora do container principal)
